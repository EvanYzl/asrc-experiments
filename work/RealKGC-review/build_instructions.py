...
def build_instructions(dataset, train_size, subgraph_type, neg_num, version):
    ...
    for pos_triple in tqdm(
        data_manager.path_set,
        ...
        if subgraph_type == "combine":
            ...
            pos_subgraph_prompt = SUBGRAPH_REASON_PROMPT.format(
                neighbor_triples="\n".join(pos_neighbor_triples),
                reasoning_paths=pos_reasoning_paths,
                test_triple=data_manager.triple_to_sentence(pos_triple)
            )
        elif subgraph_type == "neighbor-only":
            pos_neighbor_triples = data_manager.neighbor_triple_finder(pos_triple)
            pos_subgraph_prompt = NEIGHBOR_REASON_PROMPT.format(
                neighbor_triples="\n".join(pos_neighbor_triples),
                test_triple=data_manager.triple_to_sentence(pos_triple)
            )
            ...
            pos_subgraph_prompt = CLOSE_PATH_REASON_PROMPT.format(
                reasoning_paths=pos_reasoning_paths,
                test_triple=data_manager.triple_to_sentence(pos_triple)
            )
        ...
        for neg_triple in neg_samples:
            if subgraph_type == "combine":
                ...
                neg_subgraph_prompt = SUBGRAPH_REASON_PROMPT.format(
                    neighbor_triples="\n".join(neg_neighbor_triples),
                    reasoning_paths=neg_reasoning_paths,
                    test_triple=data_manager.triple_to_sentence(neg_triple)
                )
            elif subgraph_type == "neighbor-only":
                neg_neighbor_triples = data_manager.neighbor_triple_finder(neg_triple)
                neg_subgraph_prompt = NEIGHBOR_REASON_PROMPT.format(
                    neighbor_triples="\n".join(neg_neighbor_triples),
                    test_triple=data_manager.triple_to_sentence(neg_triple)
                )
            elif subgraph_type == "path-only":
                ...
                neg_subgraph_prompt = CLOSE_PATH_REASON_PROMPT.format(
                    reasoning_paths=neg_reasoning_paths,
                    test_triple=data_manager.triple_to_sentence(neg_triple)
                )
