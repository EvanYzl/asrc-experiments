...
def Path_Dictionary_for_LKG(data, appearance, obj, k_steps, num_iter, distribution, subgraph_size, phase, subgraph_total_dict, LKG):
    ...
    if LKG:
        ...
        for k in range(phase):
            ...
            cnt += len(centers)
            subgraph_total_dict = process_centers(data, obj, k_steps, num_iter, distribution, subgraph_size, centers, subgraph_total_dict, LKG)
            logger.info(f"Epoch {k + 1} Extracting Subgraph!!")
...
def build_TVGraph(nxGraph, data):
    for ex in data:
        ...
        nxGraph.add_node(t)
        nxGraph.add_edge(h, t, relation=r)
    logger.info(f'Done building Train + Valid Graph for extracting SPW of Valid Set with {len(list(nxGraph.nodes()))} nodes')
    return nxGraph
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, choices=['WN18RR', 'FB15k237', 'wiki5m_ind', 'wiki5m_trans', 'FB15k237N'], required=True, help="Dataset name")
