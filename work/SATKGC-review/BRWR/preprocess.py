...
def _load_umls_texts(path: str):
    ...
    for line in lines:
        ...
        assert len(fs) >= 2, 'Invalid line: {}'.format(line.strip())
        entity_id = fs[0]
        entity_name = fs[1] if len(fs) > 1 else ''
        entity_desc = ' '.join(fs[1:])
        umls_id2ent[entity_id] = (entity_id, entity_name.replace('__', ''), entity_desc)
    print('Load {} entities from {}'.format(len(umls_id2ent), path))

def _process_line_umls(line: str) -> dict:
    fs = line.strip().split('\t')
    assert len(fs) == 3, 'Expect 3 fields for {}'.format(line)
    head_id, relation, tail_id = fs[0], fs[1], fs[2]
    _, head, _ = umls_id2ent[head_id]
    _, tail, _ = umls_id2ent[tail_id]
    example = {'head_id': head_id,
               'head': head,
               'relation': relation,
               'tail_id': tail_id,
...
def preprocess_umls(path):
    if not umls_id2ent:
        _load_umls_texts('{}/entity2textlong.txt'.format(os.path.dirname(path)))
    lines = open(path, 'r', encoding='utf-8').readlines()
    pool = Pool(processes=args.workers)
    examples = pool.map(_process_line_umls, lines)
    pool.close()
    pool.join()
...
def main():
    ...
    for path in [args.train_path, args.valid_path, args.test_path]:
        ...
        elif args.task.lower() == 'umls':
            all_examples += preprocess_umls(path)
    ...
    if args.task.lower() == 'wn18rr':
        ...
        id2text = {k: v[2] for k, v in umls_id2ent.items()}
    else:
        assert False, 'Unknown task: {}'.format(args.task)
