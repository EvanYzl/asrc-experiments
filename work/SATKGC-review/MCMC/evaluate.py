...
def eval_single_direction(predictor: BertPredictor,
    ...
    for idx, ex in enumerate(examples):
        ...
        '''
        cur_score_info = [entity_dict.get_entity_by_idx(topk_idx).entity_id for  topk_idx in cur_topk_indices]
        
        pred_info = PredInfo(head=ex.head_id, relation=ex.relation,
                             tail=ex.tail_id, pred_tail=entity_dict.get_entity_by_idx(pred_idx).entity_id,
                             pred_score=round(cur_topk_scores[0], 4),
                             topk_score_info= cur_score_info,
                             rank=ranks[idx],
    ...
    prefix, basename = os.path.dirname(args.eval_model_path), os.path.basename(args.eval_model_path)
    split = os.path.basename(args.valid_path)
    with open('{}/eval_top100_{}_{}_{}.json'.format(prefix, split, eval_dir, basename), 'w', encoding='utf-8') as writer:
