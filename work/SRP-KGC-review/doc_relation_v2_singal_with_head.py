...
if args.use_link_graph:
    ...
    get_link_graph()
path_list = args.valid_path.split(',')
...
path2_path= '{}/allp2_w.txt'.format(root)
path3_path= '{}/allp3_w.txt'.format(root)
        ...
        if not relation.startswith('inverse '):  #forward
            ...
            for r in reversed(pathlist3):
                if r >= len(id2rel):
                    ...
                    r_str = 'inverse {}'.format(relation2str[r])
    if processed_path2:
        if any(item is None or item == 'inverse None' for item in processed_path2):
            print("Warning: processed_path2 contains None or 'inverse None' values:", ent_tuple, processed_path2)
            path_text_2 = None
        else:
            path_text_2 = ' '.join(str(item) for item in processed_path2 if item is not None)
    else:
        path_text_2 = None
    if processed_path3:
        if any(item is None or item == 'inverse None' for item in processed_path3):
            print("Warning: processed_path3 contains None or 'inverse None' values:", ent_tuple, processed_path3)
            path_text_3 = None
        else:
            path_text_3 = ' '.join(str(item) for item in processed_path3 if item is not None and item != 'inverse None')
    else:
        path_text_3 = None
    if path_text_2 is None and path_text_3 is None:
        if_path = False
        #print(11111)
    else:
        if_path = True
    return path_text_2,weight_2,path_text_3,weight_3,if_path
    #return path_text_2,weight_2,path_text_3,weight_3

class Example:

    def __init__(self, head_id, relation, tail_id=None, pred_gold_score=None, negatives=None, is_train=False, relation_id=None,**kwargs):
    ...
    def vectorize(self, is_train=False, num_negatives=1) -> dict:
        ...
        tokenizer = get_tokenizer()
        path_text_2,weight_2,path_text_3,weight_3,if_path = _get_path_inverse(self.head_id, self.tail_id,self.relation,tokenizer)
        ...
        if path_text_2 is not None:
            rp_2 = self.relation +  path_text_2
        else:
            rp_2 = self.relation
        hrp_2_encoded_inputs=_custom_tokenize(text=head_text,
                                             text_pair=rp_2)
        hrp_2_token_ids = hrp_2_encoded_inputs['input_ids']
        hrp_2_token_type_ids = hrp_2_encoded_inputs['token_type_ids']
        if path_text_3 is not None:
            rp_3 = self.relation + path_text_3
        else:
            rp_3 = self.relation
        hrp_3_encoded_inputs=_custom_tokenize(text=head_text,
                                             text_pair=rp_3)
        hrp_3_token_ids = hrp_3_encoded_inputs['input_ids']
        hrp_3_token_type_ids = hrp_3_encoded_inputs['token_type_ids']
        negative_encoded_inputs = []
        #process Path and relation
        if path_text_2 is not None:
            path_text =path_text_2
        else:
            path_text = self.relation
        path_encoded_input = _custom_tokenize2(text=path_text)
        path_token_ids = path_encoded_input['input_ids']
        path_token_type_ids = path_encoded_input['token_type_ids']

        relation_text = self.relation
        relation_encoded_input = _custom_tokenize2(text=relation_text)
        relation_token_ids = relation_encoded_input['input_ids']
        relation_token_type_ids = relation_encoded_input['token_type_ids']
        ...
        return {'hr_token_ids': hr_encoded_inputs['input_ids'],
                ...
                'hrp_2_token_ids': hrp_2_token_ids,
                'hrp_2_token_type_ids': hrp_2_token_type_ids,
                'weight_2': weight_2,
                'hrp_3_token_ids': hrp_3_token_ids,
                'hrp_3_token_type_ids': hrp_3_token_type_ids,
                'weight_3': weight_3,
...
def collate(batch_data: List[dict]) -> dict:
    ...
    hrp_2_token_ids, hrp_2_mask = to_indices_and_mask(
        [(ex['hrp_2_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    hrp_2_token_type_ids = to_indices_and_mask(
        [(ex['hrp_2_token_type_ids']) for ex in batch_data],
        need_mask=False)
    hrp_3_token_ids, hrp_3_mask = to_indices_and_mask(
        [(ex['hrp_3_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    hrp_3_token_type_ids = to_indices_and_mask(
        [(ex['hrp_3_token_type_ids']) for ex in batch_data],
        need_mask=False)
    ...
    path_token_ids,path_mask = to_indices_and_mask(
        [ex['path_token_ids'] for ex in batch_data], pad_token_id=get_tokenizer().pad_token_id)
    path_token_type_ids = to_indices_and_mask(
        [ex['path_token_type_ids'] for ex in batch_data], need_mask=False)
    relation_token_ids,relation_mask = to_indices_and_mask(
        [ex['relation_token_ids'] for ex in batch_data], pad_token_id=get_tokenizer().pad_token_id)
    relation_token_type_ids = to_indices_and_mask(
        [ex['relation_token_type_ids'] for ex in batch_data], need_mask=False)
    relation_id = [ex['relation_id'] for ex in batch_data]
    if_path = [ex['if_path'] for ex in batch_data]
    if not args.is_test and args.use_hard_negative:
        ...
        negative_token_type_ids = to_indices_and_mask(
            ...
            need_mask=False)
    weight_2=[ex['weight_2'] for ex in batch_data ]
    weight_2_mask = torch.zeros((len(weight_2), len(weight_2)))
    weight_2_mask[range(len(weight_2)), range(len(weight_2))] = torch.tensor(weight_2).float()
    weight_3=[ex['weight_3'] for ex in batch_data ]
    weight_3_mask = torch.zeros((len(weight_3), len(weight_3)))
    weight_3_mask[range(len(weight_3)), range(len(weight_3))] = torch.tensor(weight_3).float()
    ...
    batch_dict = {
        ...
        'hr_mask': hr_mask,
        "weight_2_mask":weight_2_mask,
        'hr_token_type_ids': hr_token_type_ids,
        'hrp_2_token_type_ids': hrp_2_token_type_ids,
        'hrp_2_token_ids': hrp_2_token_ids,
        'hrp_2_mask': hrp_2_mask,
        'hrp_3_token_type_ids': hrp_3_token_type_ids,
        'hrp_3_token_ids': hrp_3_token_ids,
        'hrp_3_mask': hrp_3_mask,
        "weight_3_mask":weight_3_mask ,
