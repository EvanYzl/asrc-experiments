...
from doc_relation_v2_singal_with_head import load_data, Example
from logger_config import logger
from predict_path_token_muti import BertPredictor
...
@torch.no_grad()
def compute_metrics(predictor: BertPredictor,
                    hr_tensor: torch.tensor,hp_tensor: torch.tensor,
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        # batch_score = torch.mm(hp_tensor[start:end, :], entities_tensor.t())
        #batch_score = torch.mm(hr_tensor[start:end, :], entities_tensor.t())
        batch_score = torch.mm(hp_tensor[start:end, :], entities_tensor.t())
        ...
        #if args.ind:
        #batch_score=sample_mining(batch_score,batch_target)
        ...
        if args.task.lower() != 'dbpedia500':
            ...
            tensor=hr_tensor[start:end, :]
            if args.rerank_num > 0:
                batch_score = predictor.rerank_by_path(batch_score, examples[start:end],tensor,entity_dict=entity_dict, rerank_num=args.rerank_num)
...
def predict_by_split():
    ...
    logger.info('eval_model_path: {}'.format(args.eval_model_path))
    ...
    with open('{}/metrics_{}_{}_rerank{}.json'.format(prefix, split, basename, args.rerank_num), 'w', encoding='utf-8') as writer:
...
def eval_single_direction(predictor: BertPredictor,
    ...
    hr_tensor,t_tensor ,hp_tensor,_= predictor.predict_by_examples(examples, only_head_embedding=False)
    ...
    topk_scores, topk_indices, metrics, ranks,golds,gold_scores = compute_metrics(predictor,hr_tensor=hr_tensor, hp_tensor=hp_tensor,entities_tensor=entity_tensor,
...
def sample_mining(batch_score,batch_target):
    batch_size, entity_cnt = batch_score.size()
    for i in range(batch_size):
        target_idx = batch_target[i].item()
        sample_indices = torch.randperm(entity_cnt)[:1000].tolist()
        if target_idx not in sample_indices:
            sample_indices[-1] = target_idx
        mask = torch.ones(entity_cnt, dtype=torch.bool, device=batch_score.device)
        mask[sample_indices] = 0
        batch_score[i].masked_fill_(mask, float('-inf'))
    return batch_score
