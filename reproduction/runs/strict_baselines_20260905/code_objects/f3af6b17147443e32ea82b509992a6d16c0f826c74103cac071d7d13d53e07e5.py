...
def audited_kens(path,obj):
    receipts=RUN/'results/kens_v2_prediction_checks'
    if any(not (receipts/f'{path.parent.name}_{kg}.json').exists() for kg in KGS[obj['dataset']]):
        environment=os.environ.copy();environment['PYTHONPATH']=''
        verified=subprocess.run([str(ROOT/'reproduction/envs/kens-tf210/Scripts/python.exe'),str(SUITE/'audit_kens_repaired.py'),
            '--group',str(path.parent)],env=environment,capture_output=True,text=True,encoding='utf-8',errors='replace')
        assert verified.returncode==0,'Independent KEnS checkpoint/vote replay failed: '+verified.stdout+verified.stderr
    ...
    for kg,info in obj['per_kg'].items():
        ...
        proof=json.loads((receipts/f'{path.parent.name}_{kg}.json').read_text(encoding='utf-8'))
        assert config.get('training_revision')==obj['training_revision']==proof.get('training_revision')
        assert proof.get('passed') and proof['query_sha256']==info['query_sha256'] and proof['weight_sha256']==info['weight_sha256']
        assert proof['checkpoint_sha256']=={k.replace('\\','/'):v for k,v in info['checkpoints'].items()}[f'model/{kg}.h5']
...
def collect_impl(build=True):
    ...
    def fill(ident,value,display,source_type,**meta):
        ...
        if cell_scope(ident)['scope']=='proposed_method_excluded':return
    ...
    for (method,ds,condition),by_seed in sorted(groups.items()):
        ...
        for metric in ['h1','h10'] if method=='KEnS' else ['mrr','h1','h10']:
            ...
            fill(ident,mean,rf'\({mean*100:.2f}\mathbin{{\pm}}{sd*100:.2f}\)','rerun',standard_deviation=sd,
                 ...
                 notes=('KEnS corrected TransE loss/sampling/CSLS/boosting; paper batch256 and L2=.0001; fixed released rounds; nomination-specific Hits. See KENS_REPAIR.zh.md.'
                        if method=='KEnS' else 'Raw query ranks and checkpoints validated; source implementation recorded in each config.json'))
        if table=='T3':
