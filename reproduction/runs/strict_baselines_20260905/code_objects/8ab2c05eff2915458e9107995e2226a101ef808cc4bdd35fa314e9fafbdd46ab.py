...
def prioritize(tables=None):
    ...
    active_tables=tables or manifest.get('active_tables',['T1','T2','T3','T4'])
    ...
    # Finish the in-progress three-seed KEnS DBP group, then complete fast
    # baseline families before long graph runs. Dependencies still take precedence.
    def priority(job):
        name=job['id']
        if name.startswith('kens_dbp5l_'):return (0,name)
        ...
        if name.startswith(('internal_base_','internal_teacher_','internal_controls_')):
            # Complete each dataset's three-seed shared backbone and its Table 4
            # controls before starting another dataset. Dependencies order stages.
