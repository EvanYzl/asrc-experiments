...
Budget matching to QURA is intentionally deferred. Complete prefix ranks are
saved so a future validation-only budget can be applied without selecting on
test accuracy. No QURA predictor, threshold or certificate is trained here.
"""
import argparse
...
def prefix_replay(opts,model,data,out,global_means,split):
    ...
    for name,repeat_ids in [('Global-Utility',[0]),('Random',list(range(1,11)))]:
        for fraction in [0,.1,.2,.25,.4,.5,.6,.75,.8,.9,1.]:
            ...
            for repeat in repeat_ids:
                length=np.minimum(np.floor(available_n*fraction+jitter[repeat]).astype(int),available_n)
    ...
    atomic_json(out/f'{split}_budget_grid.json',{'split':split,'entries':grid,'primary_filter':primary,
        'status':'budget scan only; final QURA-matched policy pending its validation access budget',
...
def main():
    ...
    atomic_json(out/'config.json',{'version':INTERNAL_VERSION,'dataset':opts.dataset,'seed':opts.seed,'condition':condition,'teacher_root':opts.teacher_root,
        'teacher_result_sha256':sha256(Path(opts.teacher_root)/'result.json'),'attention_input':'frozen target entity 128 + relation 128 + current source log1p sketch 512 + mean other log1p sketches 512 + other-empty bit',
        ...
        'budget_matching':'pending QURA val_select access budget; all prefix ranks and rounding jitters saved',
    ...
    for kg in DOMAINS[opts.dataset]:
        ...
        if opts.dataset=='dbp5l' and condition=='full':
    ...
    atomic_json(out/'result.json',{'status':'completed','full_data':not opts.smoke,'publication':False,'kind':'internal-control-bundle',
        ...
        'global_random_status':'full prefix and budget-grid results saved; final budget matching requires QURA validation budget'})
