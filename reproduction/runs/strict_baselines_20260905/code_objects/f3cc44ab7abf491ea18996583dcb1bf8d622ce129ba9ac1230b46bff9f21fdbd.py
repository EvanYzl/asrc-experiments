...
def train(args):
    ...
    if args.MAX_SAM < 10000000000:
        student.train_set = student.train_set[:args.MAX_SAM]
        student.valid_set = student.valid_set[:args.MAX_SAM]
        student.test_set = student.test_set[:args.MAX_SAM]
...
#    scheduler_l = get_constant_schedule_with_warmup(optimizer_l, args.steps//100)
    ...
    for step in range(1, args.steps+1):
        ...
        for gan_step in range(args.gan_steps):
            ...
            # CosineEmbeddingLoss expects one scalar label per embedding pair.
            # The released code used shape (N, 1), which fails as soon as
            # --gan_steps is non-zero; a flat (N,) target is the intended
            # all-positive pairing and is compatible with both old and new
            # PyTorch releases.
            gan_ones = torch.ones(gan_batch_size, device=device, requires_grad=False)
        ...
        if len(training_logs) > 0 and (step % args.eval_steps == 0 or step == args.steps):
            logging.info("--------------------------------------")
            # Avoid retaining the training allocator cache during the dense
            # all-entity ranking pass on Windows/WDDM.  This releases only
            # unused CUDA blocks and does not alter model state or scores.
            gc.collect()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--kge_model", type=str, default="TransE",
                        ...
                        help="KGE model")
    parser.add_argument("--MAX_SAM", type=int, default=10000000000,
                        help="subset of triples/alignment rows for smoke testing")
