...
from src.model import get_entity_layer, get_relation_layer
import numpy as np
...
class MultiModelTester:
    ...
    def get_embedding_matrix(self, model):
        return get_entity_layer(model).get_weights()[0].reshape([1, -1])
    ...
    def predict(self, h, r, mode, supporter_kg=None, voting_function=None):
        ...
        if mode == TestMode.KG0:
            ...
            top_k_indices_scores = self.target_kg.kNN_finder.predict(
                ...
                batch_size=1, verbose=0)  # [array([[id1, id2, ...]]), array([[score1, score2, ...]])]
            ...
            top_k_indices_scores = supporter_kg.kNN_finder.predict(
                [h, r, self.get_embedding_matrix(supporter_kg.model)], batch_size=1, verbose=0)
            top_k = self.flatten_kNN_finder_output(top_k_indices_scores)  # [(id1, score1), (id2, score2), ...]
            return top_k
    ...
    def __predict_by_knowledge_transfer(self, h0, r0, supporter_kg=None):
        ...
        # Unaligned target entities have no supporter representation. Exclude
        # them instead of treating their placeholder zero vectors as candidates.
        target_ids=np.array(sorted(supporter_kg.dict0to1),dtype=np.int64)
        source_ids=np.array([supporter_kg.dict0to1[int(e)] for e in target_ids],dtype=np.int64)
        entity=get_entity_layer(supporter_kg.model).get_weights()[0]
        relation=get_relation_layer(supporter_kg.model).get_weights()[0]
        query=project_t([tf.constant(entity[int(h1)][None,None,:]),tf.constant(relation[int(r1)][None,None,:])])
        distance=tf.norm(tf.constant(entity[source_ids])-tf.reshape(query,[-1]),axis=1)
        ranked=tf.math.top_k(-distance,k=min(param.k,len(target_ids)))
        t0_and_scores=[(int(target_ids[i]),float(score)) for i,score in zip(ranked.indices.numpy(),ranked.values.numpy())]
    ...
    def test_and_record_results(self, testcases):
        # The formal reproduction uses TransE.  Its projected tail is exactly
        # E[h] + R[r], so all exhaustive queries can be evaluated in bounded
        # batches instead of invoking Keras once per (model, triple).  RotatE
        # retains the original scalar path below.
        if param.knowledge == 'transe':
            return self.__test_and_record_results_transe_batched(testcases)

        return self.__test_and_record_results_scalar(testcases)

    def __test_and_record_results_scalar(self, testcases):
        time0 = time.time()
        hits = 0
    ...
    @staticmethod
    def __transe_queries(kg, heads, relations):
        entity_matrix = get_entity_layer(kg.model).get_weights()[0]
        relation_matrix = get_relation_layer(kg.model).get_weights()[0]
        heads = np.asarray(heads, dtype=np.int64)
        relations = np.asarray(relations, dtype=np.int64)
        return np.ascontiguousarray(
            entity_matrix[heads] + relation_matrix[relations],
            dtype=np.float32,
        )

    @staticmethod
    def __ranked_lists(indices, scores):
        return [
            [
                (int(entity_id), float(score))
                for entity_id, score in zip(row_indices, row_scores)
            ]
            for row_indices, row_scores in zip(indices, scores)
        ]

    def __assert_scalar_prediction_samples(
            self, heads, relations, observed, supporter_kg=None):
        """Audit batched output against the untouched Keras prediction path."""
        if len(heads) == 0:
            return
        sample_ids = np.unique(np.linspace(
            0, len(heads) - 1, num=min(4, len(heads)), dtype=np.int64
        ))
        for sample_id in sample_ids:
            if supporter_kg is None:
                reference = self.predict(
                    heads[sample_id], relations[sample_id], TestMode.KG0
                )
            else:
                reference = self.predict(
                    heads[sample_id], relations[sample_id], TestMode.Transfer,
                    supporter_kg=supporter_kg,
                )
            reference_entities = [int(entity_id) for entity_id, _ in reference]
            observed_entities = [
                int(entity_id) for entity_id, _ in observed[sample_id]
            ]
            if reference_entities != observed_entities:
                raise RuntimeError(
                    'Batched final prediction disagrees with the original '
                    f'Keras path at row {sample_id}: '
                    f'reference={reference_entities}, '
                    f'batched={observed_entities}'
                )

    def __test_and_record_results_transe_batched(self, testcases):
        samples = min(param.n_test, testcases.shape[0])
        cases = np.asarray(testcases[:samples])
        heads = cases[:, 0].astype(np.int64, copy=False)
        relations = cases[:, 1].astype(np.int64, copy=False)
        batch_size = getattr(param, 'knn_batch_size', 256)

        target_candidates = get_entity_layer(self.target_kg.model).get_weights()[0]
        target_queries = self.__transe_queries(
            self.target_kg, heads, relations
        )
        target_indices, target_scores = exact_batched_l2_knn(
            target_queries,
            target_candidates,
            param.k,
        ...
        assert_batched_knn_matches_reference(
            target_queries, target_candidates, target_indices, param.k
        )
        prediction_columns = {
            self.target_kg.lang: self.__ranked_lists(
                target_indices, target_scores
            )
        }
        self.__assert_scalar_prediction_samples(
            heads, relations, prediction_columns[self.target_kg.lang]
        )

        for supporter_kg in self.supporter_kgs:
            supporter_predictions = [[] for _ in range(samples)]
            mapped_rows = np.asarray([
                row_id for row_id, head in enumerate(heads)
                if int(head) in supporter_kg.dict0to1
            ], dtype=np.int64)
            if mapped_rows.size:
                mapped_target_heads = heads[mapped_rows]
                supporter_heads = np.asarray([
                    supporter_kg.dict0to1[int(head)]
                    for head in mapped_target_heads
                ], dtype=np.int64)
                supporter_relations = relations[mapped_rows]
                supporter_queries = self.__transe_queries(
                    supporter_kg, supporter_heads, supporter_relations
                )
                target_ids=np.array(sorted(supporter_kg.dict0to1),dtype=np.int64)
                source_ids=np.array([supporter_kg.dict0to1[int(e)] for e in target_ids],dtype=np.int64)
                filtered_candidates=get_entity_layer(supporter_kg.model).get_weights()[0][source_ids]
                count=min(param.k,len(target_ids))
                supporter_indices, supporter_scores = exact_batched_l2_knn(
                    supporter_queries,
                    filtered_candidates,
                    count,
                    batch_size=batch_size,
                ...
                mapped_predictions = self.__ranked_lists(
                    target_ids[supporter_indices], supporter_scores
                )
                for mapped_id, row_id in enumerate(mapped_rows):
                    supporter_predictions[int(row_id)] = mapped_predictions[mapped_id]

                # Compare a compact mapped subset to the repository's scalar
                # transfer path.  This verifies both head redirection and the
                # reordered candidate table, not just the distance kernel.
                self.__assert_scalar_prediction_samples(
                    mapped_target_heads,
                    supporter_relations,
                    mapped_predictions,
            ...
            prediction_columns[supporter_kg.lang] = supporter_predictions

        results = {
            'h': heads,
            'r': relations,
            't': cases[:, 2].astype(np.int64, copy=False),
        }
        results.update(prediction_columns)
