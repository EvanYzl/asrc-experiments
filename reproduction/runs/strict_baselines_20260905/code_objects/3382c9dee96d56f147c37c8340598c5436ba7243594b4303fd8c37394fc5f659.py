...
def weight_matrix_from_one_triple(row, num_entity):
    ...
    weight_matrix[np.asarray(row.get('known_true_tails',[row['t']]),dtype=np.int64)]=0
    ...
    for j, lang in enumerate(langs):
        ...
        if true_t in predictions:
            ...
            if rank > 0:
                ...
                rank_matrix[np.array(predictions[:rank]), j] = -1
        else:
            rank_matrix[np.array(predictions).astype(np.int32), j] = -1  # entities in predictions are wrongly ranked
            eligible=np.setdiff1d(np.arange(num_entity),np.asarray(row.get('known_true_tails',[true_t]),dtype=np.int64))
            rest = np.random.choice(eligible, size=max(0,int(100 / base[lang]) - len(predictions)),replace=True)
            rank_matrix[rest, j] = -1
    rank_matrix[np.asarray(row.get('known_true_tails',[true_t]),dtype=np.int64),:]=0
    return rank_matrix
