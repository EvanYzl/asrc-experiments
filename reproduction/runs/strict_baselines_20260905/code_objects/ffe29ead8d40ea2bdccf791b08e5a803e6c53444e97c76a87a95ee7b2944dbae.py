...
def audited_result(path):
    ...
    per_kg={};rows=[];checkpoints=[];hashes=[];paired={}
...
def audited_kens(path,obj):
    ds=obj['dataset'];rows=[];per_kg={};checkpoints=[];hashes=[]
    ...
    return {'method':'KEnS','dataset':ds,'seed':obj['seed'],'condition':'full','macro':macro,'rows':rows,'run_id':path.parent.name,
        'checkpoints':checkpoints,'checkpoint_hashes':hashes,'path':str(path)}
...
def collect(build=True):
    ...
    for path in sorted((RUN/'jobs').rglob('result.json')):
        ...
        audit.append({k:v for k,v in result.items() if k!='rows'});metrics_rows.extend(result['rows'])
    published=[]
    for (method,ds,condition),by_seed in sorted(groups.items()):
        if sorted(by_seed)!=SEEDS:continue
        short={'Target-only':'target','Uniform-All':'uniform','Query Attention':'attn'}.get(method,method.lower());key=DS[ds];table='T2'
        ...
        for metric in ['h1','h10'] if method=='KEnS' else ['mrr','h1','h10']:
            ...
            fill(ident,mean,rf'\({mean*100:.2f}\mathbin{{\pm}}{sd*100:.2f}\)','rerun',standard_deviation=sd,
                 ...
                 selection_protocol='val_select tail MRR, earliest maximum',candidate_scope='all target-KG entities',
        ...
        if method in ['Uniform-All','Query Attention']:
            for metric in ['mrr','ntr_all','mean_harm','ptr','coverage','edges']:
                ...
                if ds=='dbp5l' and method=='Uniform-All' and metric in ['mrr','ntr_all']:
                    scope='.'.join(condition.split('_')) if condition.startswith(('el_','ja_')) else f'dbp.{condition}'
                    identifiers.append(f'T7.{scope}.uniform.{metric}')
                ...
                for ident in identifiers:
                    fill(ident,mean,rf'\({mean*scale:.2f}\mathbin{{\pm}}{sd*scale:.2f}\)','rerun',standard_deviation=sd,
                        ...
                        aggregation='equal KG macro then seed mean and sample SD',notes='Paired raw ranks and source records revalidated')
        published.append({'method':method,'dataset':ds,'condition':condition,'seeds':SEEDS})
    collect_benchmarks(fill)
    ...
    pdf_stale=not (TABLES/'KBS_Main_Text_Tables.pdf').exists() or (TABLES/'KBS_Main_Text_Tables.pdf').stat().st_mtime < (TABLES/'values.tex').stat().st_mtime
    if build and (changed or pdf_stale):
            built=subprocess.run(['latexmk','-norc','-xelatex','-interaction=nonstopmode','-halt-on-error','-file-line-error','-outdir=build','main.tex'],cwd=TABLES,capture_output=True,text=True,encoding='utf-8',errors='replace')
    ...
    if manifest_path.exists():
        ...
        tm.update(all_experimental_values_unfilled=False,filled_cells=len(values),performance_source='local reruns only',
