...
def run_target(opts,out):
    ...
    atomic_json(out/'config.json',{'method':'SS-AGA','dataset':opts.dataset,'kg':opts.target,'seed':opts.seed,'recipe':vars(args),
        'environment':version_info(),'source_hashes':source_hashes([Path(__file__),SUITE/'common.py',ROOT/'reproduction/tools/build_ssaga_mbert_embeddings.py',source/'run_model.py',*sorted(source.glob('src/*.py'))]),
