...
def update():
    ...
    lines=['# 基线复现实验进度','',f"更新时间：{payload['updated_at']}",'',
        ...
        f"{active_label} 中可独立于 QURA 完成的单元格共 {scope.get('filled',0)+scope.get('pending',0)} 格，已填 {scope.get('filled',0)} 格，待完成 {scope.get('pending',0)} 格。我们方法相关 {scope.get('proposed_method_excluded','待核对')} 格按要求不运行。",
        ...
        f'所有性能值来自本机运行，文献值不代填。当前只填 {active_label} 的非 QURA 项，其余表格留空。','',
