from src.model import create_knowledge_model, create_alignment_model, create_vec_kNN_finder, get_entity_layer, get_relation_layer
...
class KnowledgeGraph:
    def __init__(self, lang, kg_train_data, kg_val_data, dict0to1, dict1to0, num_entity, num_relation):
        ...
        # keras model
        training_triples=np.concatenate([kg_train_data,kg_val_data]) if self.dict0to1 is not None else kg_train_data
        self.model, _, self.kNN_finder = create_knowledge_model(self.num_entity, self.num_relation,training_triples=training_triples)
    ...
    def get_embedding_matrix(self):
        ...
        """
        return get_entity_layer(self.model).get_weights()[0].reshape([1, -1])

    def get_relation_matrix(self):
        """
        return relation embedding as [n_rel, dim]
        """
        return get_relation_layer(self.model).get_weights()[0]
    ...
    def get_filtered_reordered_embedding_matrix(self, num_entity0):
        ...
        E1 = get_entity_layer(self.model).get_weights()[0]
        # Keep the repository model's embedding dtype (normally float32).
        # NumPy's default float64 doubles the memory footprint and provides no
        # extra information because every copied value originates in E1.
        E0 = np.zeros([num_entity0, param.dim], dtype=E1.dtype)  # shape [num_entity0, dim]
    ...
    def get_entity_vec(self, ent):
        ent_matrix = get_entity_layer(self.model).get_weights()[0]
        return ent_matrix[ent, :]

    def get_relation_vec(self, rel):
        rel_matrix = get_relation_layer(self.model).get_weights()[0]
    ...
    def load_model_from_json(self, model_dir):
        ...
        with open(file) as f:
            ...
            ent_embedding_layer = get_entity_layer(self.model)
            rel_embedding_layer = get_relation_layer(self.model)
