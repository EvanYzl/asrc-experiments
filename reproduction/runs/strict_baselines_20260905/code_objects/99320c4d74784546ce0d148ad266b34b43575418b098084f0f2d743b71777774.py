...
def update():
    ...
    lines=['# 基线复现实验进度','',f"更新时间：{payload['updated_at']}",'',
        f"当前任务：`{payload['current_job'] or '无'}`。正式任务完成 {counts['formal']['completed']}/{sum(counts['formal'].values())}，运行检查完成 {counts['smoke']['completed']}/{sum(counts['smoke'].values())}。正式任务数包含共享教师、内部对照和计时任务。",
        f"已验收三种子组数：{len(payload['completed_three_seed_groups'])}；已填单元格：{payload['filled_cells']}（含数据集统计）。",'',
        f"本次表格范围：应填 {scope.get('eligible_baseline_and_statistics_slots','待核对')} 格，待完成 {scope.get('pending','待核对')} 格；我们方法相关 {scope.get('proposed_method_excluded','待核对')} 格按要求不运行。",
