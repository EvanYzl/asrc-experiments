...
def cell_scope(ident):
    ...
    if table == 'T3':
        method = {'kens':'kens','align':'alignkgc','ssaga':'ssaga'}[parts[2]]
...
def update_scope():
    ...
    lines=['# 全部正文表的基线执行范围','',
           ('用户最新要求：只填写 '+ '、'.join(active_tables) +' 中不涉及 QURA 的结果。QURA 行、相关差值与区间留空；其他表格不填。' if priority_only else '用户要求：除我们提出的方法及其消融、认证、衍生差值外，完成全部现有表格中的基线实验。'),
           'Random / Global-Utility 保存完整预算前缀曲线，待 QURA 的验证集访问量冻结后确定匹配预算；此前不填其主表数值。','',
