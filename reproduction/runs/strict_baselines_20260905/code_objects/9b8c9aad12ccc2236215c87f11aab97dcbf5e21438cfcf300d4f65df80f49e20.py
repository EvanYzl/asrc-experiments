...
def update():
    ...
    payload={'updated_at':datetime.datetime.now().astimezone().isoformat(),'logical_status':'completed' if complete else ('needs_attention' if unresolved and not state.get('current_job') else 'running'),
        'current_job':state.get('current_job'),'counts':{k:dict(v) for k,v in counts.items()},'filled_cells':audit.get('filled_cells',0),
        'completed_three_seed_groups':audit.get('three_seed_groups',[]),'unresolved_failures':unresolved,'recovered_attempts':recovered,
        'remaining_dependency':'Global/Random table cells await matching to the frozen QURA validation access budget',
        'table_scope':scope,
        'jobs':rows}
    ...
    lines=['# 基线复现实验进度','',f"更新时间：{payload['updated_at']}",'',
        f"当前任务：`{payload['current_job'] or '无'}`。正式任务完成 {counts['formal']['completed']}/{sum(counts['formal'].values())}，运行检查完成 {counts['smoke']['completed']}/{sum(counts['smoke'].values())}。正式任务数包含外部基线、共享骨干、教师和内部对照。",
        f"已验收三种子组数：{len(payload['completed_three_seed_groups'])}；已填单元格：{payload['filled_cells']}（含数据集统计）。",'',
        f"Table 1–4 中可独立于 QURA 完成的单元格共 {scope.get('filled',0)+scope.get('pending',0)} 格，已填 {scope.get('filled',0)} 格，待完成 {scope.get('pending',0)} 格。我们方法相关 {scope.get('proposed_method_excluded','待核对')} 格按要求不运行。",
        f"Global/Random 的 {scope.get('waiting_qura_budget',0)} 格需等待 QURA 验证集访问量确定匹配预算，完整前缀曲线先保存。",
        '所有性能值来自本机运行，文献值不代填。当前只填 Table 1–4 的非 QURA 项，其余表格留空。','',
