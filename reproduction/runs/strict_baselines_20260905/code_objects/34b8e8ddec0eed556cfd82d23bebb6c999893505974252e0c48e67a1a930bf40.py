...
def collect_impl(build=True):
    ...
    for ident,cell in cells.items():
        ...
        if ident.startswith(('T5.','T6.')) or '.qura.' in ident or '.delta_' in ident or '.lifecycle.' in ident:
            cell['notes']='Proposed method, its effects or complete lifecycle excluded from the current run scope'
        elif '.global.' in ident or '.random.' in ident:
            cell['notes']='Full baseline budget-prefix scans queued; final cell requires QURA validation access budget'
        else:cell['notes']='Awaiting complete audited local reruns for all required seeds and target KGs'
