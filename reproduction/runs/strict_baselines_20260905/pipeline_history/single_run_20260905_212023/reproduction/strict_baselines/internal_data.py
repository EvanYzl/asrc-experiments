...
def alignments(dataset,target,condition='full'):
    ...
    for s in sources:
        ...
        if condition.startswith('corrupt'):
            ...
            for entity in bad:
                left=np.searchsorted(ordered[:,0],entity,side='left');right=np.searchsorted(ordered[:,0],entity,side='right')
                correct=set(map(int,ordered[left:right,1]))
                while int(mapping[entity]) in correct:mapping[entity]=(int(mapping[entity])+1)%source_n
                assert int(mapping[entity]) not in correct
