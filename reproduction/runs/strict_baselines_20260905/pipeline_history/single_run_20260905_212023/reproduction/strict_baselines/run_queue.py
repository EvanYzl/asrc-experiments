...
def main():
    ...
    try:
        while True:
            ...
            if not ready:
                superseded={j['supersedes'] for j in manifest['jobs'] if j.get('supersedes')}
                state['status']='complete' if all(state['jobs'][j['id']]['status']=='completed' for j in manifest['jobs'] if j['id'] not in superseded) else 'needs_attention'
