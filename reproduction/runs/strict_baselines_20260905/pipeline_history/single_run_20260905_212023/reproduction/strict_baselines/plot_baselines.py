...
def export_profile_metrics(records,ready):
    ...
    for key,values in grouped.items():
        assert sorted(int(row['seed']) for row in values)==[17,29,43]
        ...
        for metric in ['mrr','h1','h10']:
            samples=np.array([row[metric] for row in values]);result[metric+'_mean']=float(samples.mean());result[metric+'_sd']=float(samples.std(ddof=1))
        means.append(result)
