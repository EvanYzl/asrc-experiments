...
TRAINING_REVISION='kens-transe-paper-loss-v2'
...
def main():
    ...
    for kg in DOMAINS[args.dataset]:
        ...
        assert per_kg[kg].get('training_revision')==TRAINING_REVISION,'Legacy KEnS output cannot be reused after training repair'
    ...
    save(out/'result.json',{'status':'completed','purpose':'smoke' if args.smoke else 'formal','full_data':not args.smoke,
        ...
        'protocol':'kens-nomination-hits-v2-validation-only','training_revision':TRAINING_REVISION,'wall_time_s':time.time()-started,
...
def run_target(args,out):
    ...
    code_hashes={str(f.relative_to(ROOT)):digest(f) for f in [Path(__file__),source/'run.py',*sorted((source/'src').glob('*.py'))]}
    objects=ROOT/'reproduction/runs/strict_baselines_20260905/code_objects';objects.mkdir(exist_ok=True)
    for relative,checksum in code_hashes.items():
        artifact=objects/(checksum+'.py')
        if not artifact.exists():artifact.write_bytes((ROOT/relative).read_bytes())
    ...
    trainargs=module.parse_args(['--target_language','ja','--knowledge_model','transe','--seed',str(args.seed),
        '--dim','300','--batch_size','256','--learning_rate','0.001','--transe_margin','0.3',
        '--reg_scale','0.0001','--base_align_step','5','--knowledge_step_ratio','2','--round','2'])
    ...
    known_true={key:set(values) for key,values in known.items()}
    for h,r,t in val[['h','r','t']].values:known_true.setdefault((int(h),int(r)),set()).add(int(t))
    val_entities['known_true_tails']=[sorted(known_true[int(h),int(r)]) for h,r in val[['h','r']].values]
    for kg in langs:
        val_entities[kg]=[[int(e) for e,s in pairs if int(e) not in (known.get((int(h),int(r)),set())-{int(t)})]
            for (h,r,t),pairs in zip(val[['h','r','t']].values,val[kg])]
    ...
    save(out/'config.json',{'method':'KEnS','dataset':args.dataset,'kg':args.target,'seed':args.seed,'recipe':vars(trainargs),
        'raw_file_sha256':raw_hashes,'source_hashes':code_hashes,
        'tensorflow':tf.__version__,'numpy':np.__version__,'device':'CPU','supporter_facts':'train + public validation once',
        'training_revision':TRAINING_REVISION,
        'paper_reference':'https://aclanthology.org/2020.findings-emnlp.290.pdf, equations 1-2; section 4.1',
        'training_recipe':'paper d300/lr.001/batch256/margin.3/L2.0001; released two rounds, ten knowledge epochs and five alignment epochs per round',
        'selection':'fixed two rounds; entity weights fitted on public validation only',
        'repairs':['negative loss scores h+r against corrupted tail','per-query loss keeps batch axis independent',
            'uniform per-query negatives cover all entity IDs and exclude known training positives',
            'supporter validation facts appended once','CSLS forward maximum updated',
            'unaligned supporter candidate placeholders excluded','embedding layers identified by names',
            'boosting excludes known true-true pairs and runs once per KG',
            'read headerless alignment TSV without dropping first row','ensemble prior uses validation instead of test',
            'zero validation hit prior floored at 1/number_validation_queries','filter retains current gold'],
