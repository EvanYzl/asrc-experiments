...
def main():
    ...
    for group in sorted((RUN/'jobs').glob('kens_*/result.json')):
        ...
        if result.get('status')!='completed' or not result.get('full_data') or result.get('training_revision'):continue
    ...
    payload={'status':'passed','model_or_protocol_changed':False,
        ...
        'interpretation':'Legacy prediction consistency only. These outputs are withdrawn (publication=false) after training-formula bugs were identified; repaired v2 reruns are audited separately.',
        'rows':rows}
    ...
    lines=['# KEnS 预测复核','',
        ...
        '此报告仅记录修复前的执行一致性。后续发现训练公式错误，旧结果已撤下（publication=false）；修复版见 KENS_REPAIR.zh.md 和 kens_v2_prediction_checks/。','',
