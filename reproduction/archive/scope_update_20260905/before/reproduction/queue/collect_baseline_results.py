...
def parse_kens(text: str) -> Dict[str, Optional[float]]:
    return {
        "mrr": None,
        "hits1": last_float(r"Hits@1 \(\d+ triples\):\s*([0-9.eE+-]+)", text),
        "hits3": last_float(r"Hits@3 \(\d+ triples\):\s*([0-9.eE+-]+)", text),
        "hits10": last_float(r"Hits@10 \(\d+ triples\):\s*([0-9.eE+-]+)", text),
    }
