...
def main():
    ...
    for group in groups:
        ...
        for metric in metrics:
            ...
            if old['value']:
                if old['notes'].startswith(('INTERIM_SINGLE_SEED:','INTERIM_TWO_SEED:')):
                    assert prefix=='T3.epkg.ssaga' and old['seed'] in ['17','17;29']
                    ...
                    assert partial['status'] in ['interim_single_seed','interim_two_seed'] and partial['seeds']==[int(s) for s in old['seed'].split(';')]
                    label='17 (1/3)' if partial['seeds']==[17] else '17,29 (2/3)'
                    assert old['value']==(label if metric=='paper_table' else str(partial['mean'][metric]))
                    if metric!='paper_table':
                        previous_values=[group['per_seed'][str(s)][metric] for s in partial['seeds']]
                        assert partial['mean'][metric]==float(np.mean(previous_values))
                        if len(previous_values)==2:
                            assert float(old['standard_deviation'])==partial['sample_sd'][metric]==float(np.std(previous_values,ddof=1))
                        else:assert not old['standard_deviation']
    ...
    for number in (2, 3):
        ...
        if number == 2:
            ...
            if any(cid.startswith('T3.epkg.ssaga.') for cid in updates):
                text=text.replace('Completed rows report three-seed mean $\\pm$ sample SD (17, 29, 43).','Values are local three-seed means (17, 29, 43); SS-AGA includes sample SD.')
                text=text.replace(' SS-AGA on E-PKG reports seed 17 only (1/3); seeds 29/43 and the sample SD are pending.','')
                text=text.replace(' SS-AGA on E-PKG reports two-seed mean $\\pm$ sample SD (17, 29; 2/3); seed 43 is pending.','')
    ...
    if any(cid.startswith('T3.epkg.ssaga.') for cid in updates):
        ...
        manifest['interim_groups']=[]
        manifest['all_registered_three_seed_groups_complete']=True
        manifest['task_scope']='All registered main-table three-seed groups are complete; no further experiments'
