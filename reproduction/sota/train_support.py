...
def main():
    p=argparse.ArgumentParser();p.add_argument('--id',required=True);p.add_argument('--smoke',action='store_true');cli=p.parse_args()
    ...
    config={'method':'ASRC registered component/perturbation control','recipe':recipe,'id':cli.id,'smoke_only':cli.smoke,'alignment':audit,
            ...
            'source_hashes':{p.name:sha256(p) for p in [Path(__file__),Path(__file__).with_name('frozen_data.py'),Path(__file__).with_name('support_data.py')]},
