...
def main():
    ...
    for group in groups:
        ...
        for metric, cid in ids.items():
            ...
            if (method, dataset) == ('IMKGC', 'depkg'):
                row['notes'] += ' Original seed17 continued on the server from its full-state checkpoint after 26 of 50 rounds; source/path and preserved-history provenance audited.'
            updates[cid] = row
            lookup[cid] = row
    ...
    note += (r'Full target candidates, tail prediction and ascending entity-ID ties are fixed. '
             ...
             + (r'The three unknown WK3l graph baselines keep the comparison provisional. ' if len(groups) == 9 else
    ...
    manifest.update(total_cells=len(original), filled_cells=sum(bool(r['value']) for r in lookup.values()),
                    ...
                    task_scope=('All nine registered graph-baseline groups complete; ASRC and supporting results frozen' if len(groups) == 9 else 'Publish accepted complete graph-baseline groups; remaining registered repeats continue; ASRC and supporting results frozen'))
