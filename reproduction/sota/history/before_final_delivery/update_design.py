...
'''
acceptance=ROOT/'reproduction/sota/SINGLE_SEED_ACCEPTANCE.json'
if acceptance.exists():
    report=json.loads(acceptance.read_text(encoding='utf-8'))
    assert report['verified_datasets']==4 and report['seed']==17
    text=text.replace('状态：种子 17 按数据集顺序验收；复用已完成初筛，仅使用单种子结果，未宣称 SOTA 或方法创新。',
        '状态：四数据集种子 17 已依次完成冻结、单次测试和结果锁定；'+report['verdict']+'。不宣称完整可比 SOTA 或方法创新。')
    lines=['## 已锁定的单种子测试结果','',
        '所有数字为全量测试逐查询排名重算的等权宏平均，单位为 %。只报告种子 17，无多种子标准差。']
    labels={'dbp5l':'DBP-5L','depkg':'E-PKG','dwy':'DWY','wk3l':'WK3l-15k'}
    for ds,d in report['datasets'].items():
        m=d['macro'];wins=sum(x['strict_win'] for x in d['comparisons'])
        lines+=['',f"{labels[ds]}：MRR {m['mrr']*100:.2f}，H@1 {m['h1']*100:.2f}，H@10 {m['h10']*100:.2f}；{wins}/3 项严格超过当前参考。验证选定 epoch {d['best_epoch']}，对齐共享开关 {d['recipe']['sharing']}。"]
    lines+=['',f"结论：{report['verdict']}，{report['strict_wins']}/12 项严格超过；验收标准仍为至少 10/12。测试查询顺序、三种过滤的原始排名、宏平均和冻结的代码/检查点/参考哈希已核验。星号暂代值、未知图基线及重复次数差异仍保留，因此不宣称完整可比 SOTA。",
        '', '四套结果已锁定，不再训练或重测。本阶段停止实验，完成文档与原始产物回传核验后等待用户指令，不自动补基线或其他种子。详细证据见 reproduction/sota/SINGLE_SEED_ACCEPTANCE.json、SINGLE_SEED_COMPARISON.csv、single_seed/ 各数据集的 freeze.json、LOCKED_RESULT.json 和原始 test_queries.npz。', '']
    text=text.replace('## 参考文献','\n'.join(lines)+'\n## 参考文献')
