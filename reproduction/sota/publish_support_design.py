...
for ds,name in names.items():
    numbers=[cell('T4',ds+'.independent','mrr'),cell('T4',ds+'.shared','mrr')]
    numbers += [cell('T4',ds+'.shared',k,unit='pp' if k=='delta' else '%') for k in ['delta','ntr','ptr','harm']]
    rows.append(row(name,numbers+['Independent' if ds=='depkg' else 'Shared']))
titles.append(table(4,'Paired transfer effects of aligned parameter sharing.',r'L{20mm}*{6}{C}L{19mm}',
    r'Dataset & \makecell{Independent\\MRR} & \makecell{Shared\\MRR} & $\Delta$MRR & NTR & PTR & \makecell{Mean RR\\harm} & \makecell[l]{ASRC\\mode}',rows,
    r'Three-seed mean $\pm$ sample SD. MRR, rates and mean RR harm are in \%; differences are percentage points. Both representations use the same reciprocal ComplEx/N3 backbone. All transfer statistics compare Shared ($r_S$) with the seed-matched Independent rank ($r_I$): $\Delta$MRR is Shared minus Independent, NTR is $\Pr(r_S>r_I)$, PTR is $\Pr(r_S<r_I)$, and mean RR harm is $100\mathbb{E}[\max(1/r_I-1/r_S,0)]$. Compute per KG, then macro-average. The last column identifies the previously frozen validation choice used by ASRC in Table~\ref{tab:2}; its scores equal the corresponding MRR column. Self-comparisons, which are identically zero, and duplicate ASRC score rows are omitted from this compact presentation; all raw statistics are retained. No test-based mode choice is made.'))
rows=[]
...
preserved=[approved.get(r['cell_id'],r) for r in old if r['table_id'] in ['T1','T2','T3']];rows=list(preserved)
...
for cid,r in approved.items():
    values=[line for line in values if not line.startswith('\\SetResult{'+cid+'}')]
    if r['placeholder_kind']=='TXT':
        assert cid.startswith('T3.') and cid.endswith('.paper_table')
        assert r['value']==r['seed']=='17;29;43' or (cid=='T3.epkg.ssaga.paper_table' and ((r['seed']=='17' and r['value']=='17 (1/3)' and r['notes'].startswith('INTERIM_SINGLE_SEED:')) or (r['seed']=='17;29' and r['value']=='17,29 (2/3)' and r['notes'].startswith('INTERIM_TWO_SEED:'))))
        values.append('\\SetResult{'+cid+'}{'+r['value']+'}')
        continue
    shown=f"{100*float(r['value']):.2f}"
    if r['placeholder_kind']=='PM':
        assert (r['seed']=='17;29;43' or (cid.startswith('T3.epkg.ssaga.') and r['seed']=='17;29' and r['notes'].startswith('INTERIM_TWO_SEED:'))) and r['standard_deviation']
        shown+=r'\mathbin{\pm}'+f"{100*float(r['standard_deviation']):.2f}"
    else:assert r['placeholder_kind']=='V'
...
for cell_spec in registered:
    ...
    if value is not None:
        ...
        decimals=0 if key=='qps' else (1 if key in ['rss_mib','vram_mib'] else 2)
...
if (tables/'table2_highlights.tex').exists():
...
for number in [1,2,3]:
    ...
    if name in approved_sources:
        ...
        if sha256(tables/name)!=spec['sha256']:shutil.copy2(source,tables/name)
    else:assert sha256(tables/name)==sha256(archive/'tables'/name)
...
print(json.dumps({'total_cells':len(rows),'support_cells':len(registered),'filled_support':sum(bool(r['value']) for r in rows[len(preserved):]),'approved_baseline_updates_preserved':len(approved),'approved_table_sources_preserved':list(approved_sources)}))
