...
report={'passed':True,'support_final':not args.allow_pending,'total_cells':len(rows),'support_cells':len(support),
    ...
    'interim_external_cells':[r['cell_id'] for r in rows if r['notes'].startswith('INTERIM_SINGLE_SEED:')],
    'pending_three_seed_cells':[r['cell_id'] for r in rows if r['notes'].startswith('INTERIM_SINGLE_SEED:') or (not r['value'] and r['table_id']=='T3')],
