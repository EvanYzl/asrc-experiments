...
def main():
    ...
    teacher.update(method=new_label,role='source_teachers',source=relative(source),per_kg=values,
                   ...
                   checkpoint_sha256={'en':teacher['checkpoint_sha256'],'fr':result['domains']['fr']['checkpoint_sha256']})
    ...
    data.update(timestamp=dt.datetime.now(dt.timezone.utc).isoformat(),completion_batch=relative(BASE/'manifest.json'))
    ...
    receipt={'timestamp':data['timestamp'],'status':'ready_to_compile','numeric_cells':378,'new_numeric_cells':56,
