...
if mode=='pack':
    ...
    for name in ['import_table2_screenshot.py','import_imkgc_dbp_seed17.py','publish_graph_repeat_tables.py','refresh_table2_highlights.py',
                 'audit_main_completion.py','publish_available_main_tables.py',
                 'audit_ssaga_interim_seed17.py','publish_ssaga_interim_seed17.py','audit_ssaga_two_seed.py','publish_ssaga_two_seed.py']:
        if (base/name).exists():files.add(base/name)
    main_completion=ROOT/'reproduction/language_table_seed17/main_completion_20260907'
    for name in ['PLAN.json','PUBLICATION_RESULTS.json','PUBLICATION_SOURCE_RECEIPT.json','CURRENT_COMPARISON.json','MAIN_PUBLICATION.json',
                 'INTERIM_SEED17_RESULTS.json','INTERIM_TWO_SEED_RESULTS.json']:
        if (main_completion/name).exists():files.add(main_completion/name)
    for name in ['publication_acceptance','publication_evidence']:
        if (main_completion/name).is_dir():
            files.update(path for path in (main_completion/name).rglob('*') if path.is_file() and path.suffix in ['.json','.py'])
