...
existing_values=dict(re.findall(r'^\\SetResult\{([^}]+)\}\{(.*)\}$',(tables/'values.tex').read_text(encoding='utf-8'),flags=re.M))
value_lines=['% From verified raw results; user reference lines are explicitly separate.']
for row in rows:
    if row['value']=='':continue
    cid=row['cell_id'];kind=row['placeholder_kind']
    if '.asrc.' not in cid and cid in existing_values:
        value_lines.append('\\SetResult{'+cid+'}{'+existing_values[cid]+'}')
        continue
    if kind=='TXT':
        formatted=row['value'].replace('_',r'\_').replace('%',r'\%').replace('&',r'\&')
        value_lines.append('\\SetResult{'+cid+'}{'+formatted+'}')
        continue
    value=float(row['value'])
    if cid.startswith('T1.') and kind=='N':formatted=f'{value:,.0f}'
...
s=s.replace(r'L{34mm}*{9}{C}',r'L{20mm}*{9}{C}')
