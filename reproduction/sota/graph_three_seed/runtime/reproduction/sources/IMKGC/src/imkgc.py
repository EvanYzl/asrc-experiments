...
class IMKGC(nn.Module):
    ...
    def forward_GNN_embedding(self, graph_input_list, kg_index):
        ...
        x_gnn_output = self.encoder_KG(
            x_features, graph_input.edge_index, graph_input.edge_kg_index,
            edge_beta_r, edge_relation_embedding, graph_input.y,
            graph_input.num_size,
        )
        x_gnn_output_views = self.layer_norm_gnn(x_gnn_output).reshape(
            num_views, batch_size, -1
        )

        for i in range(num_views):
            x_gnn_output = x_gnn_output_views[i]
        ...
        x_gnn_output_all = x_gnn_output_views.permute(1, 0, 2) # [batch_size, num_kgs, out_dim]
    ...
    def forward_kg(self, h_graph, sample, t_graph, t_neg_graph, kg_index):
        ...
        if self.num_kgs > 1:
            ...
            info_contrastive = torch.tensor([0.], device=self.device)

        bs = h.size(0)
        target = torch.tensor([-1], dtype=torch.long, device=self.device)
        loss_assist_kg = torch.tensor([0.], device=self.device)
        loss_assist_kld = torch.tensor([0.], device=self.device)

        loss_cur_kg = torch.tensor([0.], device=self.device)
    ...
    def forward_kg_combined(self, entity_graph, sample, kg_index):
        """Encode disconnected head, tail and negative groups together."""
        batch_size = sample.shape[0]
        entity_embeddings, experts = self.forward_GNN_embedding(
            entity_graph, kg_index
        )
        ...
        split_z = [z.split(batch_size, dim=0) for z in experts['z']]
        split_dist = [
            tuple(
                torch.distributions.Normal(mu, sigma)
                for mu, sigma in zip(
                    distribution.loc.split(batch_size, dim=0),
                    distribution.scale.split(batch_size, dim=0),
                )
            )
            for distribution in experts['x_gnn_dist']
        ]
        h_experts = {
            'z': [groups[0] for groups in split_z],
            'x_gnn_dist': [groups[0] for groups in split_dist],
        }
        t_experts = {
            'z': [groups[1] for groups in split_z],
            'x_gnn_dist': [groups[1] for groups in split_dist],
        }
        t_neg_experts = {
            'z': [groups[2] for groups in split_z],
            'x_gnn_dist': [groups[2] for groups in split_dist],
        }

        r = self.rel_embedding_layer(sample[:, 1])
        r, _, vq_loss = self.rvq(r)
        r = r.unsqueeze(1)
        h = h.unsqueeze(1)
        t = t.unsqueeze(1)
        t_neg = t_neg.unsqueeze(1)
        ...
        if self.num_kgs > 1:
            info_contrastive = (
                self.mutual_info(h_experts['z'])
                + self.mutual_info(t_experts['z'])
                + self.mutual_info(t_neg_experts['z'])
            )
        else:
            info_contrastive = torch.tensor([0.], device=self.device)
        ...
        normal_dist = torch.distributions.Normal(
            torch.zeros_like(h_experts['z'][0]),
            torch.ones_like(h_experts['z'][0]),
        )
        for view in range(self.num_kgs):
            h_z = h_experts['z'][view].unsqueeze(1)
            t_z = t_experts['z'][view].unsqueeze(1)
            t_neg_z = t_neg_experts['z'][view].unsqueeze(1)
            pos_loss, neg_loss = self.decoder(h_z, r, t_z, t_neg_z)
            view_loss = self.criterion(
                pos_loss.expand(batch_size, batch_size).reshape(-1),
                neg_loss.transpose(-1, -2).expand(
            ...
            kld_loss = (
                kl_divergence(
                    h_experts['x_gnn_dist'][view], normal_dist
                ).sum(-1).mean(dim=0)
                + kl_divergence(
                    t_experts['x_gnn_dist'][view], normal_dist
                ).sum(-1).mean(dim=0)
                + kl_divergence(
                    t_neg_experts['x_gnn_dist'][view], normal_dist
                ).sum(-1).mean(dim=0)
            ) / 3
            if view == kg_index:
                loss_cur_kg = view_loss
            else:
                loss_assist_kg += view_loss
            loss_assist_kld += kld_loss
        ...
        return {
            'kg_loss': kg_loss,
            'kld_loss': loss_assist_kld,
            'kg_cur_loss': loss_cur_kg,
            'kg_assist_loss': loss_assist_kg,
            'info_contrastive': info_contrastive,
            'vq_loss': vq_loss,
        }
