...
def ranking_all_batch(predicted_t, embedding_matrix, k = None, define_score = None, d_r = None):
    ...
    if (predicted_t.ndim == 3 and predicted_t.size(1) == 1
        ...
        # IMKGC uses a fixed Euclidean TransE decoder.  Avoid materializing
        # the broadcast [batch, candidates, dim] difference tensor.
...
def create_subgraph_list(edge_index, edge_type, total_num_nodes, num_hops, k):
    ...
    for i in tqdm(node_list):
        ...
        # Preserve the exact selected edge order while avoiding two temporary
        # full-size int64 tensors for every center entity.
