...
class MKGDataset:
    ...
    def create_KG_objects_and_subgraph(self):
        ...
        packed_cache_paths = [
            os.path.join(self.data_dir, graph_dir, 'kg{}_k_subgraph_list.graph'.format(index))
            for index in range(self.num_kgs)
        ]
        if all(os.path.exists(path) for path in packed_cache_paths):
            print('All packed domain-view caches found; skip base subgraph rebuilding.')
            subgraph_list = []
        elif not os.path.exists(subgraph_list_path):
