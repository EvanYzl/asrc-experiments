...
class MKGDataset:
    ...
    def create_KG_objects_and_subgraph(self):
        ...
        graph_cache_dir = os.path.join(self.data_dir, graph_dir)
        expected_view_caches = [
            os.path.join(graph_cache_dir, f'kg{i}_k_subgraph_list.graph')
            for i in range(self.num_kgs)
        ]

        if not all(os.path.exists(path) for path in expected_view_caches):
            edge_index, edge_type = get_all_edges(
                self.kg_dir, kg_objects_dict, all_entity_global_index,
            )
