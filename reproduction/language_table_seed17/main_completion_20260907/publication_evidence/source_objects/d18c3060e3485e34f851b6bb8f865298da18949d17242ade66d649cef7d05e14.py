...
from src.checkpointing import load_round_checkpoint, save_round_checkpoint
import logging
import argparse
import random
...
def parse_args(args=None):
    ...
    parser.add_argument('--resume_checkpoint', default='', type=str,
                        help='atomic full-state checkpoint for round-boundary resume')
    parser.add_argument('--clamp', default=3., type=float, help = '')
    parser.add_argument('--lw', type=str, default='n',choices=['y', 'n'], help='')
    parser.add_argument('--fuse_entity_groups', action='store_true',
                        help='encode disconnected head/tail/negative groups in one GNN call')
...
def train_kgs(args, all_kgs, kg_objects_dict, kgname2idx, optimizer, num_epoch, model, scheduler=None):
    ...
    for one_epoch in range(num_epoch): # 3
        ...
        kld_loss_sum = torch.zeros((), device=args.device)
        all_loss_sum = torch.zeros((), device=args.device)
        loss_count = 0
        ...
        for i in range(max_data):
            ...
            for lang in all_kgs:
                ...
                if args.fuse_entity_groups:
                    entity_indices = torch.cat((
                        kg_each[:, 0], kg_each[:, 2], t_neg_index,
                    ), dim=0)
                    entity_graph_batch_list = nodes_to_k_graph(
                        kg.k_subgraph_list, entity_indices, args.device
                    )
                else:
                    h_graph_batch_list = nodes_to_k_graph(
                        kg.k_subgraph_list, kg_each[:, 0], args.device
                    )
                    t_graph_batch_list = nodes_to_k_graph(
                        kg.k_subgraph_list, kg_each[:, 2], args.device
                    )
                    t_neg_graph_batch_list = nodes_to_k_graph(
                        kg.k_subgraph_list, t_neg_index, args.device
                    )
                ...
                optimizer.zero_grad()
                if args.fuse_entity_groups:
                    total_loss = model.forward_kg_combined(
                        entity_graph_batch_list, kg_each, kg_index
                    )
                else:
                    total_loss = model.forward_kg(
                        h_graph_batch_list, kg_each, t_graph_batch_list,
                        t_neg_graph_batch_list, kg_index
                    )
                ...
                all_loss_sum += loss.detach().mean()
                kg_loss_sum += total_loss['kg_loss'].detach().mean()
                kld_loss_sum += total_loss['kld_loss'].detach().mean()
                loss_count += 1
        ...
        logging.info('Epoch {:d} [Train KG Loss: all_loss {:.6f}, kg_loss {:.6f}, kld_loss {:.6f}]'.format(
            one_epoch,
            (all_loss_sum / loss_count).item(),
            (kg_loss_sum / loss_count).item(),
            (kld_loss_sum / loss_count).item(),
        ))
        logging.info('\n')
...
def main(args):
    ...
    start_round = 0
    resume_state = load_round_checkpoint(
        args.resume_checkpoint, model, optimizer, scheduler, all_kgs, args.device
    )
    if resume_state is not None:
        start_round = resume_state['start_round']
        all_kgs = resume_state['ordered_domains']
        best_mrr = resume_state['best_mrr']
        best_epoch = resume_state['best_epoch']
        best_result = resume_state['best_result']
    ...
    for i in range(start_round, args.round):
        ...
        with torch.no_grad():
            ...
            for lang in all_kgs_out:
                logging.info('{} filterd: {:.8f}, {:.8f}, {:.8f}'.format(lang, best_result[lang][0], best_result[lang][1], best_result[lang][2]))
            save_round_checkpoint(
                args.resume_checkpoint, model, optimizer, scheduler, i,
                all_kgs, best_mrr, best_epoch, best_result,
            )
