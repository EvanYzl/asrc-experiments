...
# Reuse the dependency directory used by the existing local graph experiments.
# Use validated server dependencies.
from common import *
...
def main():
    ...
    parser.add_argument('--dataset',choices=['dbp5l','depkg','dwy','wk3l'],required=True)
    ...
    source=BASE/'sources'/opts.method
    ...
    dm_path=BASE/'sources/DMKGC/src/utils.py'
    ...
    instances=[]
    data_path=prepare_data(opts.dataset, f'train_only_k10_h2_{opts.method}_s{opts.seed}')
    ...
    module.get_language_list=lambda directory:['fr','en'] if opts.dataset=='wk3l' else list(DOMAINS[opts.dataset])
    rounds=opts.rounds or ({'dbp5l':80,'depkg':50,'dwy':50,'wk3l':50}[opts.dataset] if opts.method=='LSMGA' else (50 if opts.dataset=='depkg' else 30))
    if opts.smoke:rounds=1
    command=['--dataset','dbp5l' if opts.method=='LSMGA' and opts.dataset in ['dwy','wk3l'] else opts.dataset,
    ...
    class StrictTester(OriginalTester):
        ...
        def score_fn(self,kgname):
            ...
            def score(batch):
                ...
                if opts.dataset=='wk3l' and kgname=='en':ids[:,1]+=self.data['fr']['relations']
                heads=table[ids[:,0]]
        ...
        def test(self,is_val=True,is_filtered=False,**kw):
            ...
            mm=macro_metrics({k:v['metrics']['select'] for k,v in results.items() if k in DOMAINS[opts.dataset]})
    ...
    with ResourceTrace(out/'resources.csv') as trace:
        ...
        atomic_json(out/'FINAL_EVALUATION_FREEZE.json', {'seed':opts.seed, 'completed_rounds':rounds, 'selected_evaluation':state['evaluation_index'], 'checkpoint_sha256':sha256(out/'best.pt'), 'selection':'FR val_select only', 'test_targets':['fr','en']})
        with torch.no_grad():
            test=tester.evaluate_split('val_select' if opts.smoke else 'test',save=True)
            selection=tester.evaluate_split('val_select',save=True)
    per_kg={kg:result['metrics'][result['primary_filter']] for kg,result in test.items() if kg in DOMAINS[opts.dataset]}
