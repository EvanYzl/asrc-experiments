...
class ParseData(object):
    def __init__(self, args):
        ...
        self.args = args
        self.MAX_SAM = getattr(args, 'MAX_SAM', 10000000000)
    ...
    def create_KG_objects_and_alignment(self):
        ...
        # Preserve every graph field while removing the very large Python
        # object overhead of two Data objects per entity.  The packed
        # container remains index-compatible and supports sparse replacement
        # when self-learning later adds new alignment edges.
        for kg in kg_objects_dict.values():
            kg.subgraph_list_kg = pack_subgraph_list(kg.subgraph_list_kg)
            kg.subgraph_list_align = pack_subgraph_list(kg.subgraph_list_align)
        gc.collect()
    ...
    def load_kg_data(self, language):
        ...
        train_df = pd.read_csv(join(self.data_kg, language + '-train.tsv'), sep='\t', header=None,names=['v1', 'relation', 'v2'])[:self.MAX_SAM]
        val_df = pd.read_csv(join(self.data_kg, language + '-val.tsv'), sep='\t', header=None,names=['v1', 'relation', 'v2'])[:self.MAX_SAM]
        test_df = pd.read_csv(join(self.data_kg, language + '-test.tsv'), sep='\t', header=None,names=['v1', 'relation', 'v2'])[:self.MAX_SAM]
        ...
        triples_train = train_df.values.astype(np.int64)
        triples_val = val_df.values.astype(np.int64)
        triples_test = test_df.values.astype(np.int64)
