...
def get_KNN_batch(query_original_and_bert, embedding_matrix_original_and_bert, k,device,batch_size):
    ...
    for i in range(total_num_batch):
        # Exhaustive Euclidean search, but keep only this block's Top-k.  The
        # original retained every full distance block and concatenated an
        # N_query x N_candidate matrix after also creating a 3-D broadcast.
        distance_0 = torch.cdist(query_emb0[i], candidate_emb0, p=2)
        _, top_k_indexes = torch.topk(-distance_0, k=k)
        top_k_table.append(top_k_indexes.cpu())
        del distance_0, top_k_indexes

    top_k_indexes = torch.cat(top_k_table, dim=0)
...
def select_csls_mutual_links(
        e0_neighborhood_indexes, e1_neighborhood_indexes,
        e0_neighborhood_cos_table, e1_neighborhood_cos_table,
        e0_neighborhood_cos, e1_neighborhood_cos,
        aligned0_entities, aligned1_entities, num_entity0, num_entity1):
    """Vectorized equivalent of the repository's CSLS mutual-link loop.

    The published loop performs membership tests and scalar comparisons on
    CUDA tensors one entity at a time.  This routine preserves its candidate
    masks, first-on-tie behavior, and ascending e0 output order while removing
    tens of thousands of device synchronizations per language pair.
    """
    device = e0_neighborhood_indexes.device
    aligned0_mask = torch.zeros(num_entity0, dtype=torch.bool, device=device)
    aligned1_mask = torch.zeros(num_entity1, dtype=torch.bool, device=device)
    aligned0_mask[aligned0_entities.to(device=device, dtype=torch.long)] = True
    aligned1_mask[aligned1_entities.to(device=device, dtype=torch.long)] = True

    e0_candidates = (~aligned0_mask).nonzero(as_tuple=False).view(-1)
    if e0_candidates.numel() == 0:
        return torch.empty((0, 2), dtype=torch.long)

    forward_neighbors = e0_neighborhood_indexes[e0_candidates]
    # This intentionally matches the released code: forward candidates are
    # retained when they ARE present in aligned1_entities.
    forward_valid = aligned1_mask[forward_neighbors]
    forward_scores = (
        2.0 * e0_neighborhood_cos_table[e0_candidates]
        - e1_neighborhood_cos[forward_neighbors]
    ).masked_fill(~forward_valid, -torch.inf)
    forward_best_score, forward_best_pos = forward_scores.max(dim=1)
    forward_best = forward_neighbors.gather(
        1, forward_best_pos.view(-1, 1)
    ).view(-1)
    forward_best = torch.where(
        torch.isfinite(forward_best_score),
        forward_best,
        torch.full_like(forward_best, -1),
    )

    reverse_valid = ~aligned0_mask[e1_neighborhood_indexes]
    reverse_scores = (
        2.0 * e1_neighborhood_cos_table
        - e0_neighborhood_cos[e1_neighborhood_indexes]
    ).masked_fill(~reverse_valid, -torch.inf)
    reverse_best_score, reverse_best_pos = reverse_scores.max(dim=1)
    reverse_best = e1_neighborhood_indexes.gather(
        1, reverse_best_pos.view(-1, 1)
    ).view(-1)
    reverse_best = torch.where(
        torch.isfinite(reverse_best_score),
        reverse_best,
        torch.full_like(reverse_best, -1),
    )

    has_forward = forward_best >= 0
    safe_forward = forward_best.clamp_min(0)
    mutual = has_forward & (reverse_best[safe_forward] == e0_candidates)
    if not mutual.any():
        return torch.empty((0, 2), dtype=torch.long)
    return torch.stack(
        (e0_candidates[mutual], forward_best[mutual]), dim=1
    ).cpu()
...
class SSAGA(nn.Module):
    def __init__(self, args, entity_bert_emb,num_relations, num_entities, num_KGs):
        ...
        '''
        self.args = args
        self.batch_size = args.knn_batch_size
    ...
    def forward_align_RANKING(self, e0_graph, e1_graph, e1_graph_neg):
        ...
        target = torch.full_like(align_loss_pos, -1)
    ...
    def extend_seed_align_links(self, kg0, kg1, seeds, device, k_csls=3):
        ...
        search_device = torch.device(
            getattr(self.args, 'self_learning_device', 'cpu')
        )
        if search_device.type == 'cuda' and not torch.cuda.is_available():
            raise RuntimeError('CUDA self-learning requested but CUDA is unavailable')

        aligned0_entities = torch.unique(
            seeds[:, 0], return_inverse=False
        ).to(search_device)
        aligned1_entities = torch.unique(
            seeds[:, 1], return_inverse=False
        ).to(search_device)
        ...
        if kg0.computed_entity_embedidng_align == None:
            computed = self.get_kg_embeddings_matrix(
                kg0, self.args.precompute_batch_size,
                self.args.device, is_kg=False
            )
            kg0.computed_entity_embedidng_align = computed.to(search_device)
            del computed

        if kg1.computed_entity_embedidng_align == None:
            computed = self.get_kg_embeddings_matrix(
                kg1, self.args.precompute_batch_size,
                self.args.device, is_kg=False
            )
            kg1.computed_entity_embedidng_align = computed.to(search_device)
            del computed
        ...
        emb_bert0 = torch.index_select(
            self.entity_bert_emb, 0, entity_index0
        ).to(search_device)
        emb_bert1 = torch.index_select(
            self.entity_bert_emb, 0, entity_index1
        ).to(search_device)



        # Step1 : find KNN for each e0:using batch: TODO: try not directly compute the whole cosine table.
        e0_neighborhood_indexes = get_KNN_batch((embedding_matrix0, emb_bert0), (embedding_matrix1, emb_bert1),k=k_csls, device=search_device,batch_size=self.batch_size)  # [N_0, k_csls], [N0,N1]
        e1_neighborhood_indexes = get_KNN_batch((embedding_matrix1, emb_bert1), (embedding_matrix0, emb_bert0),k=k_csls, device=search_device,batch_size=self.batch_size)  # [N_0, k_csls], [N0,N1]
        ...
        # Step3: same mutual-CSLS selection as the released scalar loop, in a
        # small number of tensor operations rather than per-entity GPU syncs.
        csls_links = select_csls_mutual_links(
            e0_neighborhood_indexes,
            e1_neighborhood_indexes,
            e0_neighborhood_cos_table,
            e1_neighborhood_cos_table,
            e0_neighborhood_cos,
            e1_neighborhood_cos,
            aligned0_entities,
            aligned1_entities,
