...
def parse_args(args=None):
    ...
    parser.add_argument('--test_batch_size', default=0, type=int,
                        help="evaluation query batch; 0 uses batch_size")
    parser.add_argument('--precompute_batch_size', default=0, type=int,
                        help="entity-embedding precompute batch; 0 uses batch_size")
    parser.add_argument('--knn_batch_size', default=0, type=int,
                        help="self-learning exact kNN query batch; 0 uses batch_size")
    parser.add_argument('--self_learning_device', default='cpu', choices=['cpu', 'cuda'],
                        help="device for exact kNN/CSLS self-learning; CPU avoids Windows display-driver resets")
    ...
    parser.add_argument('--MAX_SAM', default=10000000000, type=int,
                        help="subset of triples for smoke-test debugging; default keeps all data")
...
def train_align_batch(args,align_data,optimizer,kg0,kg1,model):
    ...
    for one_epoch in range(args.epoch2):
        ...
        for align_each in align_data_loader:
            ...
            # Preserve the repository's random-number consumption.  The
            # negative graph is not used by ``forward_align`` and therefore
            # need not be materialized on the GPU.
            ...
            for start in range(0, batch_size, micro_batch_size):
                ...
                e0_graph = nodes_to_graph(
                    kg0.subgraph_list_align, align_each[start:stop, 0]
                ).to(args.device)
                e1_graph = nodes_to_graph(
                    kg1.subgraph_list_align, align_each[start:stop, 1]
                ).to(args.device)
                loss = model.forward_align(e0_graph, e1_graph, None)
                weight = float(stop - start) / float(batch_size)
                (loss * weight).backward()
                batch_loss += loss.item() * weight
                del loss, e0_graph, e1_graph
                torch.cuda.empty_cache()
            optimizer.step()
            align_loss.append(batch_loss)
            del e1_neg_index
            torch.cuda.empty_cache()
...
def train_kg_batch(args,kg,optimizer,num_epoch,model):
    ...
    for one_epoch in range(num_epoch):
        ...
        for kg_batch_each in kg_batch_generator:
            ...
            for start in range(0, batch_size, micro_batch_size):
                stop = min(start + micro_batch_size, batch_size)
                h_graph = nodes_to_graph(
                    kg.subgraph_list_kg, kg_batch_each[start:stop, 0]
                ).to(args.device)
                t_graph = nodes_to_graph(
                    kg.subgraph_list_kg, kg_batch_each[start:stop, 2]
                ).to(args.device)
                t_neg_graph = nodes_to_graph(
                    kg.subgraph_list_kg, t_neg_index[start:stop]
                ).to(args.device)
                kg_micro_batch = kg_batch_each[start:stop].to(args.device)
                loss = model.forward_kg(
                    h_graph, kg_micro_batch, t_graph, t_neg_graph
                )
                weight = float(stop - start) / float(batch_size)
                (loss * weight).backward()
                batch_loss += loss.item() * weight
                del loss, h_graph, t_graph, t_neg_graph, kg_micro_batch
                torch.cuda.empty_cache()
            optimizer.step()
            kg_loss.append(batch_loss)
            del t_neg_index
            torch.cuda.empty_cache()
...
def main(args):
    ...
    if args.precompute_batch_size <= 0:
        args.precompute_batch_size = args.batch_size
    if args.knn_batch_size <= 0:
        args.knn_batch_size = args.batch_size
    ...
    # SSAGA owns the feature tensor after construction; do not retain a second
    # top-level NumPy reference for the rest of the 25-round run.
    del entity_bert_emb
        ...
        if (i>=args.burin_in_epoch) and (i%args.generation_freq==0):
            ...
            with torch.no_grad():
                for (kg0_name, kg1_name) in seeds_all:
                    ...
                    # Every language pair is independent after its links are
                    # written back. Release temporary native/PyTorch buffers
                    # before starting the next exact all-entity search.
                    gc.collect()
                    if torch.cuda.is_available():
