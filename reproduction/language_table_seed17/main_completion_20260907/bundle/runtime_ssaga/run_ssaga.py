...
def prepare(dataset):
    ...
    if not features.exists():
        if (raw/features.name).exists():
            ...
            subprocess.run([sys.executable,str(BASE/'tools/build_ssaga_mbert_embeddings.py'),
    ...
    if (dest/'feature_manifest.json').exists():
...
def run_target(opts,out):
    ...
    source=BASE/'sources/SS-AGA';sys.path.insert(0,str(source))
    ...
    atomic_json(out/'config.json',{'method':'SS-AGA','dataset':opts.dataset,'kg':opts.target,'seed':opts.seed,'recipe':vars(args),
        'environment':version_info(),'source_hashes':source_hashes([Path(__file__),SUITE/'common.py',BASE/'tools/build_ssaga_mbert_embeddings.py',source/'run_model.py',*sorted(source.glob('src/*.py'))]),
        'dataset_hash':data['manifest']['dataset_hash'],'feature_manifest':str(data_path/'feature_manifest.json'),'feature_manifest_sha256':sha256(data_path/'feature_manifest.json'),
