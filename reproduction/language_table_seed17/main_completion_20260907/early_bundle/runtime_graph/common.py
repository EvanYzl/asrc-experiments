...
BASE = Path('/root/zhishitupui/reproduction/main_tables_completion_20260907')
...
def load_kg(dataset, kg):
    ...
    files = {str(p.relative_to(ROOT)).replace('/', chr(92)): sha256(p) for p in [*paths.values(), entity_path, relation_path]}
...
def source_hashes(paths):
    result={};objects=BASE/'code_objects';objects.mkdir(exist_ok=True)
    for p in paths:
