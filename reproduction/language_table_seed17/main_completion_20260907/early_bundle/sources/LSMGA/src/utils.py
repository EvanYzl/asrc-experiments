...
def ranking_all_batch(predicted_t, embedding_matrix, k = None):
    ...
    # Same exhaustive Euclidean metric without materializing a
    # [batch, num_total_entities, dim] repeated tensor.
...
def get_kg_edges_for_each(kg_dir, language):
    ...
    sender_node_list = train_df['head'].values.astype(np.int64).tolist()
    sender_node_list += train_df['tail'].values.astype(np.int64).tolist()
    
    receiver_node_list = train_df['tail'].values.astype(np.int64).tolist() 
    receiver_node_list += train_df['head'].values.astype(np.int64).tolist()

    
    edge_relation_list = train_df['relation'].values.astype(np.int64).tolist()
    edge_relation_list += train_df['relation'].values.astype(np.int64).tolist()
...
def create_subgraph_list(edge_index, edge_type, total_num_nodes, num_hops, k):
    ...
    for i in tqdm(node_list):
        ...
        # Preserve the exact selected edge order with a lower-memory boolean
        # selection (important on the 32 GB local reproduction machine).
        edge_attr = edge_type[edge_masks][:k]
...
def nodes_to_k_graph(k_subgraph_list, node_index, device, shuffle=False):
    ...
    if isinstance(k_subgraph_list, dict) and k_subgraph_list.get('format') == 'packed_subgraphs_v1':
        # Materialize the packed mini-batch directly instead of recreating one
        # Python ``Data`` object per entity and asking PyG's DataLoader to
        # concatenate them again.  The masks preserve graph-major ordering,
        # while ``node_offsets`` performs exactly the edge-index increment
        # implemented by ``MyData.__inc__``/PyG batching.
        ...
        for kg_view in range(num_kgs):
            ...
            max_nodes = k_subgraph_list['x'].shape[2]
            node_mask = torch.arange(max_nodes).unsqueeze(0) < num_nodes.unsqueeze(1)
            x = k_subgraph_list['x'][rows, kg_view].long()[node_mask]

            max_edges = k_subgraph_list['edge_attr'].shape[2]
            edge_mask = torch.arange(max_edges).unsqueeze(0) < num_edges.unsqueeze(1)
            ...
            edge_index_padded = k_subgraph_list['edge_index'][rows, kg_view].long()
            edge_index_padded = edge_index_padded + node_offsets[:, None, None]
