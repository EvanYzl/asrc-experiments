...
# The existing graph dependency directory also supplies numba for ATransN.
# Use the previously validated server environment.
from common import *
...
def main():
    ...
    seed_all(opts.seed)
    source=BASE/'sources/ATransN/src';sys.path.insert(0,str(source))
    ...
    previous=BASE/'atransn_recipe.json'
    config=json.loads(previous.read_text(encoding='utf-8'))
    config.update(seed=opts.seed,teacher_data_path=str(ROOT/'data/raw/atransn/WK3l-15k_FR'),
                  student_data_path=str(ROOT/'data/raw/atransn/WK3l-15k_EN_F'),
                  shared_entity_path=str(BASE/'fr_en_aligned_entity_id.txt'),
    ...
    args=SimpleNamespace(**config);data=load_kg('wk3l','en')
    ...
    with ResourceTrace(out/'resources.csv') as trace:
        ...
        kgout=out/'en';kgout.mkdir(exist_ok=True)
        atomic_json(out/'FINAL_EVALUATION_FREEZE.json', {'seed':opts.seed, 'completed_steps':args.steps, 'checkpoint_sha256':sha256(out/'best.pt'), 'selection':'EN val_select only', 'direction':'FR-to-EN'})
    ...
    atomic_json(out/'result.json',{'status':'completed','purpose':'smoke' if opts.smoke else 'formal','full_data':not opts.smoke,
        ...
        'per_kg':{'en':result},'selection':selection,'checkpoint':str(out/'best.pt'),'checkpoint_sha256':sha256(out/'best.pt'),
