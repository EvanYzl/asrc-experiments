...
def train(args):
    ...
    if args.MAX_SAM < 10000000000:
        data.train_set = data.train_set[:args.MAX_SAM]
        data.valid_set = data.valid_set[:args.MAX_SAM]
        data.test_set = data.test_set[:args.MAX_SAM]
    ...
    for step in range(1, args.steps+1):
        ...
        if len(training_logs) > 0 and (step % args.eval_steps == 0 or step == args.steps):
            logging.info("--------------------------------------")
            # Windows/WDDM charges cached CUDA allocations against the system
            # commit limit.  Release only unused cache before the dense
            # all-entity ranking pass; live model tensors and results are
            # unchanged.
            gc.collect()
