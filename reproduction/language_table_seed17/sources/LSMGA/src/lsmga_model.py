...
class LSMGA(nn.Module):
    ...
    def forward_kg(self, h_graph, sample, t_graph, t_neg_graph, kg_index):
        ...
        total_loss = self.released_ranking_loss(pos_loss, neg_loss)
        
        return total_loss


    def released_ranking_loss(self, pos_loss, neg_loss):
        """Preserve the released PyTorch 1.10 broadcasting, including gradients.

        Upstream supplies [B, 1], [B] and [1], producing a [B, B] loss.
        Explicit broadcasting satisfies modern PyTorch's shape checks without
        replacing that objective with a different per-example ranking loss.
        """
        target = torch.tensor([-1], dtype=torch.long, device=pos_loss.device)
        positive, negative, target = torch.broadcast_tensors(pos_loss, neg_loss, target)
        return self.criterion(positive, negative, target)
    ...
    def forward_kg_combined(self, entity_graph, sample, kg_index):
        ...
        pos_loss = self.define_loss([t, projected_t])
        neg_loss = torch.mean(self.define_loss([t, t_neg]), dim=-1)
        return self.released_ranking_loss(pos_loss, neg_loss)
