...
class GNN(nn.Module):
    ...
    def get_real_index(self, y, s):
        # For graph i, PyG concatenation shifts its local centre ``y[i]`` by
        # the total node count of all preceding graphs.  The released loop
        # performs thousands of scalar GPU updates per training step; one
        # exclusive prefix sum is the exact same calculation.
        y = y.reshape(-1)
        s = s.reshape(-1)
        node_base = torch.cat((
            torch.zeros(1, dtype=s.dtype, device=s.device),
            torch.cumsum(s, dim=0)[:-1],
        ))
        return y + node_base
