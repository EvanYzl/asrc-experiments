...
def main():
    ...
    complete = bool(data.get('completion_batch'))
    assert complete or data.get('partial_completion_batch')
    if complete:
        assert all(r['status']=='completed' for rs in data['rows'].values() for r in rs)
    ...
    new='TransE (teacher) uses the source-teacher recipe (margin 4, batch 1024) in both languages.'
    if complete:
        new+=' English ATransN uses FR-to-EN transfer. Graph methods use a WK3l data-format adaptation with fixed recipes; one FR-validation-selected checkpoint serves both languages.'
    ...
    new=('All displayed results are complete seed-17 runs, verified against saved ranks. Missing baselines were trained with fixed recipes and validation-only checkpoint selection; existing ASRC and baseline results were reused.' if complete else
         'P: seed-17 result pending; NR: no accepted result. English TransE, DistMult, RotatE and the FR teacher are now complete. All filled values were verified against saved ranks; checkpoints were selected on validation data. Remaining results are pending.')
