...
class DMKGC(nn.Module):
    ...
    def forward_GNN_embedding(self, graph_input_list, kg_index, info_dict=None):
        ...
        loss_recon_all = torch.tensor([0.], device=self.device)
        loss_reg_all = torch.tensor([0.], device=self.device)
        loss_dn_all = torch.tensor([0.], device=self.device)
        num_views = len(graph_input_list)
        bs = graph_input_list[0].y.shape[0]
        ...
        x_gnn_output_all = x_gnn_output.reshape(num_views, bs, -1)
    ...
    def forward_kg_combined(self, entity_graph, sample, kg_index):
        """Encode head, true-tail and negative-tail components together.

        The input rows are ordered as three equal groups. Message passing and
        attention remain independent because every entity subgraph is a
        disconnected component. The stochastic diffusion draws have the same
        distribution as the released three-call path, while using one larger
        kernel sequence. Auxiliary MSE losses are multiplied by three because
        the released path sums three equal-sized group means.
        """
        ...
        return {
            'kg_loss': kg_loss,
            'loss_recon': experts['loss_recon'] * 3,
            'loss_dn': experts['loss_dn'] * 3,
            'loss_reg': experts['loss_reg'] * 3,
        }
