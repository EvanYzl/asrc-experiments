"""Fill provenance-verified full-domain results under the selected repetition policy."""
...
def collect_impl(build=True):
    ...
    for ident,cell in cells.items():
        ...
        spec=cell_scope(ident,manifest)
    ...
    def fill(ident,value,display,source_type,**meta):
        ...
        if cell_scope(ident,manifest)['scope']=='proposed_method_excluded':return
    ...
    for (method,ds,condition),by_seed in sorted(groups.items()):
        seeds=required_seeds(manifest,method,ds,condition)
        if not set(seeds)<=set(by_seed):continue
        ...
        if method in ['AlignKGC','SS-AGA']:
            ...
            short={'AlignKGC':'align','SS-AGA':'ssaga'}[method]
        for metric in ['mrr','h1','h10']:
            if condition!='full':continue
            ident=f'{table}.{key}.{short}.{metric}'
            if ident not in cells:continue
            data=[by_seed[s]['macro'][metric] for s in seeds];mean,sd=summarize(data)
            display=rf'\({mean*100:.2f}\)' if sd is None else rf'\({mean*100:.2f}\mathbin{{\pm}}{sd*100:.2f}\)'
            fill(ident,mean,display,'rerun',standard_deviation='' if sd is None else sd,
                 run_id=';'.join(by_seed[s]['run_id'] for s in seeds),seed=';'.join(map(str,seeds)),unit='fraction displayed as percent',
                 ...
                 selection_protocol='val_select tail MRR, earliest maximum',
                 candidate_scope='all target-KG entities',
                 aggregation=('FR only' if ds=='wk3l' else 'equal KG macro')+('; single fixed-seed run; SD unavailable' if sd is None else '; then three-seed mean and sample SD'),
                 ...
                 notes='Raw query ranks and checkpoints validated; source implementation recorded in each config.json'+('; single-run stage requested by user; additional seeds deferred' if sd is None else ''))
        if table=='T3':
    ...
    dump(result_dir/'table_fill_audit.json',{'filled_cells':len(values),'published_groups':published,
        'three_seed_groups':[g for g in published if len(g['seeds'])==3],
        'single_run_groups':[g for g in published if len(g['seeds'])==1],
        'accepted_runs':audit,'rejected_runs':failures,'proposed_method_excluded':True})
    if metrics_rows:
    ...
    if manifest_path.exists():
        ...
        tm.update(all_experimental_values_unfilled=not any(not k.startswith('T1.') for k in values),
                  ...
                  published_groups=published,three_seed_groups=[g for g in published if len(g['seeds'])==3],
                  single_run_groups=[g for g in published if len(g['seeds'])==1],fill_audit=str(result_dir/'table_fill_audit.json'))
