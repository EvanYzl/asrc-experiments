...
def extend():
    ...
    add('smoke_internal_benchmark','benchmark_internal.py',['--dataset','dbp5l','--seed','17','--smoke',
        '--teacher-root',str(RUN/'smoke/smoke_internal_teacher')],True,['smoke_internal_teacher'])
    for method,datasets in [('DMKGC',['depkg','dwy']),('LSMGA',['depkg','dwy']),('IMKGC',['depkg','dwy'])]:
        for ds in datasets:
            ident=f'smoke_{method.lower()}_{ds}'
            add(ident,'run_graph_baseline.py',['--method',method,'--dataset',ds,'--seed','17','--smoke'],True)
            if method=='IMKGC' and ds=='dwy':
                for seed in [17,29,43]:
                    add(f'imkgc_dwy_s{seed}','run_graph_baseline.py',
                        ['--method',method,'--dataset',ds,'--seed',str(seed)],depends=[ident])
            for job in jobs:
    ...
    for method in ['DMKGC','IMKGC']:
        old=f'smoke_{method.lower()}_depkg';replacement=old+'_recipe_r2'
        add(replacement,'run_graph_baseline.py',
            ['--method',method,'--dataset','depkg','--seed','17','--smoke'],True)
        for job in jobs:
            if job['id']==replacement:job['supersedes']=old
            if job['id'].startswith(f'{method.lower()}_depkg_s'):
                job['depends_on']=list(dict.fromkeys([d for d in job.get('depends_on',[]) if d!=old]+[replacement]))
    for ds in ['dbp5l','depkg','dwy']:
        replacement=f'smoke_lsmga_{ds}_release_loss_r3'
        old=f'smoke_lsmga_{ds}'+('_r2' if ds=='dwy' else '')
        add(replacement,'run_graph_baseline.py',
            ['--method','LSMGA','--dataset',ds,'--seed','17','--smoke'],True)
        for job in jobs:
            if job['id']==replacement:job['supersedes']=old
            if job['id'].startswith(f'lsmga_{ds}_s'):job['depends_on']=[replacement]
    ...
    def priority(j):
        ...
        if name.startswith(('alignkgc','internal_')):return 3
        return 4
