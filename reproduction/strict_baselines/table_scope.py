...
from publication_policy import required_seeds
...
def cell_scope(ident,manifest=None):
    ...
    if table == 'T2':
        ...
        seeds=required_seeds(manifest,{'lsmga':'LSMGA','dmkgc':'DMKGC','imkgc':'IMKGC'}.get(method,method),ds)
        spec['jobs']=[f'{method}_{ds}_s{s}' for s in seeds]
        if len(seeds)==1:spec['reason']+='; single fixed-seed run, extra seeds deferred and no standard deviation reported'
...
def update_scope():
    ...
    for cell in cells:
        ident = cell['cell_id']; spec = cell_scope(ident,manifest)
    ...
    payload = {'updated_at':datetime.datetime.now().astimezone().isoformat(),
        ...
        'all_required_jobs_queued':not awaiting_adaptation, 'priority_tables_complete':counts['pending']==0 and counts['waiting_qura_budget']==0,
        ...
        'by_table':{k:dict(v) for k,v in sorted(by_table.items())}, 'missing_jobs':missing + awaiting_adaptation,
    ...
    if batch:
        ...
        payload['method_batch']={'methods':batch['methods'],'job_ids':batch['job_ids'],
            'seeds':batch['seeds'],'repetition_policy':batch.get('repetition_policy','three_seeds'),
    ...
    lines += ['',f"当前表格基线与统计共 {payload['eligible_baseline_and_statistics_slots']} 格：已填 {counts['filled']} 格，待运行 {counts['pending']} 格，另有 {counts['waiting_qura_budget']} 格等待 QURA 验证集预算。",
              ...
              'IMKGC-DWY 已纳入统一 db/wk/yg 公开划分的本批次复现；适配核验与短检查均不作为正式表格数值。']
    if batch:
