...
class CollectArgs(NamedTuple):
    ...
    edge_kg_index: torch.Tensor
    edge_beta_r: torch.Tensor
    edge_relation_embedding: torch.Tensor
    ...
    if not torch.jit.is_scripting() and not is_compiling():
        for hook in self._propagate_forward_pre_hooks.values():
            hook_kwargs = dict(
                ...
                edge_beta_r=edge_beta_r,
                edge_relation_embedding=edge_relation_embedding,
                residual=residual,
                num_nodes=num_nodes,
            )
            ...
            if res is not None:
                ...
                x = hook_kwargs['x']
                edge_kg_index = hook_kwargs['edge_kg_index']
                edge_beta_r = hook_kwargs['edge_beta_r']
                edge_relation_embedding = hook_kwargs['edge_relation_embedding']
                residual = hook_kwargs['residual']
                num_nodes = hook_kwargs['num_nodes']
        ...
        if not torch.jit.is_scripting() and not is_compiling():
            for hook in self._message_forward_pre_hooks.values():
                hook_kwargs = dict(
                    x_j=kwargs.x_j,
                    x_i=kwargs.x_i,
                    edge_index_i=kwargs.edge_index_i,
                    edge_kg_index=kwargs.edge_kg_index,
                    edge_beta_r=kwargs.edge_beta_r,
                    edge_relation_embedding=kwargs.edge_relation_embedding,
                ...
                if res is not None:
                    kwargs = CollectArgs(
                        x_j=hook_kwargs['x_j'],
                        x_i=hook_kwargs['x_i'],
                        edge_index_i=hook_kwargs['edge_index_i'],
                        edge_kg_index=hook_kwargs['edge_kg_index'],
                        edge_beta_r=hook_kwargs['edge_beta_r'],
                        edge_relation_embedding=hook_kwargs['edge_relation_embedding'],
                        num_nodes=hook_kwargs['num_nodes'],
