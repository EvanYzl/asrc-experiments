...
def get_entity_layer(model):
    return next(layer for layer in model.layers if layer.name.startswith('kens_entity_'))


def get_relation_layer(model):
    return next(layer for layer in model.layers if layer.name.startswith('kens_relation_'))
...
def create_knowledge_model(num_entity, num_relation, relation_layer=None, training_triples=()):
    """
    ...
    if param.knowledge == 'rotate':  # double embedding
        ...
        entity_embedding_layer = keras.layers.Embedding(input_dim=num_entity,
                                                        name='kens_entity_'+str(keras.backend.get_uid('kens_entity')),
        ...
        rel_embedding_layer = keras.layers.Embedding(input_dim=num_relation,
                                                     name='kens_relation_'+str(keras.backend.get_uid('kens_relation')),
    ...
    # negative sampling
    rand_negs = FilteredTailSampler(num_entity,num_relation,
        positive_keys(training_triples,num_entity,num_relation),param.neg_per_pos,
        name='generate_negative_t_samples')([input_h,input_r,input_t])
    ...
    neg_losses = loss_layer([projected_t, neg_ts])
    ...
    if param.knowledge == 'rotate':
        ...
        neg_loss1 = keras.layers.Lambda(lambda x: tf.reduce_mean(x, axis=-1, keepdims=True), output_shape=(1,),
        ...
        neg_loss = keras.layers.Lambda(lambda x: tf.reduce_mean(x, axis=-1, keepdims=True), output_shape=(1,),
...
def create_alignment_model(knowledge_model0, knowledge_model1):
    ...
    embedding_layer0 = get_entity_layer(knowledge_model0)
    embedding_layer1 = get_entity_layer(knowledge_model1)
...
def extract_entity_embedding_matrix(knowledge_model):
    ...
    :return:
    """
    return get_entity_layer(knowledge_model).get_weights()[0].reshape([1, -1])
...
def exact_batched_l2_knn(
        query_matrix, candidate_matrix, k, batch_size=256,
        return_scores=False):
    """Return exact Euclidean kNN without per-entity Keras calls.

    The original implementation calls ``Model.predict`` once for every query
    entity.  That produces the right neighbors, but the framework setup cost
    dominates the actual distance computation on modern TensorFlow.  Scikit-
    TensorFlow matrix multiplication evaluates the same exhaustive squared
    Euclidean search in bounded batches and returns only the exact top-k
    indices.  Squared Euclidean distance has exactly the same ordering as the
    Euclidean norm used by the repository.  When requested, scores are
    returned in the repository's original form (negative Euclidean distance).

    ``batch_size`` changes memory/runtime only; it does not change the search
    space or introduce an approximate index.
    """
    queries = np.ascontiguousarray(query_matrix, dtype=np.float32)
    candidates = np.ascontiguousarray(candidate_matrix, dtype=np.float32)
    if queries.ndim != 2 or candidates.ndim != 2:
        raise ValueError('query_matrix and candidate_matrix must both be rank-2')
    if queries.shape[1] != candidates.shape[1]:
        raise ValueError('query and candidate embedding dimensions must match')
    if not 0 < k <= candidates.shape[0]:
        raise ValueError('k must be between 1 and the candidate count')

    candidate_tensor = tf.convert_to_tensor(candidates, dtype=tf.float32)
    candidate_norm = tf.reduce_sum(tf.square(candidate_tensor), axis=1)
    candidate_transpose = tf.transpose(candidate_tensor)
    neighbors = np.empty((queries.shape[0], k), dtype=np.int32)
    scores = (
        np.empty((queries.shape[0], k), dtype=np.float32)
        if return_scores else None
    )
    for start in range(0, queries.shape[0], batch_size):
        stop = min(start + batch_size, queries.shape[0])
        query_tensor = tf.convert_to_tensor(queries[start:stop], dtype=tf.float32)
        query_norm = tf.reduce_sum(tf.square(query_tensor), axis=1, keepdims=True)
        squared_distance = (
            query_norm + candidate_norm[tf.newaxis, :]
            - 2.0 * tf.matmul(query_tensor, candidate_transpose)
        )
        top_k = tf.math.top_k(-squared_distance, k=k, sorted=True)
        neighbors[start:stop] = top_k.indices.numpy()
        if return_scores:
            # Round-off in q^2 + c^2 - 2qc can make an exact zero a tiny
            # negative value.  Clamp before sqrt, matching a real L2 norm.
            scores[start:stop] = (-tf.sqrt(
                tf.maximum(-top_k.values, 0.0)
            )).numpy()
    if return_scores:
...
def assert_batched_knn_matches_reference(
        queries, candidate_matrix, batched_neighbors, k, sample_count=8):
    """Hard-check against the repository's direct TF norm/top-k operations."""
    if queries.shape[0] == 0:
        return
    sample_ids = np.unique(np.linspace(
        0, queries.shape[0] - 1,
        num=min(sample_count, queries.shape[0]), dtype=np.int64
    ))
    candidate_tensor = tf.convert_to_tensor(candidate_matrix, dtype=tf.float32)
    for query_id in sample_ids:
        query_tensor = tf.convert_to_tensor(queries[query_id], dtype=tf.float32)
        distance = tf.norm(candidate_tensor - query_tensor, axis=1)
        reference = tf.nn.top_k(-distance, k=k, sorted=True).indices.numpy()
        observed = batched_neighbors[query_id]
        if not np.array_equal(reference, observed):
            raise RuntimeError(
                'Batched exact kNN disagrees with the original TF norm/top-k path '
                f'at query {query_id}: reference={reference.tolist()}, '
                f'batched={observed.tolist()}'
            )
...
def extend_seed_align_links(kg0, kg1, seed_links):
    ...
    max_scan = getattr(param, 'max_entity_scan', 10000000000)
    scan_entity0 = min(kg0.num_entity, max_scan)
    scan_entity1 = min(kg1.num_entity, max_scan)

    # Find the same exhaustive Euclidean neighborhoods as the original code,
    # but in bounded batches rather than one Keras ``predict`` call per entity.
    # The sampled reference checks below use the repository's untouched
    # tf.norm/tf.nn.top_k operations and abort if ordered top-k indices differ.
    knn_batch_size = getattr(param, 'knn_batch_size', 256)
    e0_neighbors = exact_batched_l2_knn(
        embedding_matrix0_reshaped[:scan_entity0],
        embedding_matrix1_reshaped,
        k_csls,
        batch_size=knn_batch_size,
    )
    e1_neighbors = exact_batched_l2_knn(
        embedding_matrix1_reshaped[:scan_entity1],
        embedding_matrix0_reshaped,
        k_csls,
        batch_size=knn_batch_size,
    )
    ...
    e0_neighborhood[:scan_entity0, :] = e0_neighbors
    e1_neighborhood[:scan_entity1, :] = e1_neighbors
    ...
    for true_e0 in range(scan_entity0):
