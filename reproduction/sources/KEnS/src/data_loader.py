...
def load_align_links(link_dir, target_lang, src_lang):
    ...
    if os.path.exists(join(link_dir, file1)):
        ...
        source2target = source2target_link_df.values.astype(np.int64)
        # swap the two columns
        target2source = np.ones(source2target.shape, dtype=np.int64)
        ...
        target2source = pd.read_csv(path, sep='\t', header=None).values.astype(np.int64)
    else:
...
def take_seed_align_links(link_df, lang1, lang2, seed_ratio):
    ...
    :return:
    """
    links = link_df[[lang1, lang2]].dropna().values.astype(np.int64)
    # entity size
