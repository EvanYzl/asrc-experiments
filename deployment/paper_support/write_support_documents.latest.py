...
('Table 4：同骨干迁移对照','四数据集各比较 Independent、Always shared 和冻结 ASRC。按同种子同查询配对秩，报告 MRR、相对 Independent 的 MRR 差、负迁移率 NTR=P(r>r_I)、正迁移率 PTR=P(r<r_I)，以及平均倒数排名伤害 E[max(1/r_I−1/r,0)]。后四项先逐 KG 统计再宏平均。ASRC 行复用对应共享/独立结果；Independent 自比较项为定义上的零。共享即使平均提高 MRR，也可能伤害部分查询，不能解释成逐查询安全保证。'),
...
if result:
    ...
    sections.insert(-2,('本轮结果与解释','\n\n'.join(findings)+'\n\n'+interpretation+'\n\n以上均来自预注册的固定对照。三种子标准差反映本次训练重复性，不能替代跨数据集外推或显著性检验。原始结果和逐查询配对数据保留在 paper_support 中。'))
